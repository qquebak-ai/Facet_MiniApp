...
function сообщениеСвопа(card, сумма, своп) {
  ...
  return [
    ...
    // Комиссию показываем процентом и в токенах: голое число рядом с
    // суммой покупки читалось как её половина.
    `Комиссия пула ${своп.процентКомиссии.toFixed(2)}% · ${число(своп.комиссияБиржи)} ${тикер}`,
    `Худший расклад: ${число(своп.минимум)} ${тикер} (запас ${своп.запасПроцентов.toFixed(1)}% на проскальзывание)`,
    "",
