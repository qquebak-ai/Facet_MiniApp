...
async function handleMenu(message) {
  await tg("sendMessage", {
    ...
    text: [
      "<b>Mintly</b>",
      "",
      "Ищи токен кнопкой ниже — по тикеру или адресу. В карточке будут цена, график и кнопки покупки.",
      "",
      "Если привычнее командами:",
      "<code>/token PRSM</code> — карточка токена",
      "<code>/buy EQAw…cuNT 5</code> — купить на 5 TON",
      "<code>/sell PRSM 100</code> — продать 100 токенов",
      "",
      "Покупка идёт только по адресу контракта: один и тот же тикер может носить сколько угодно токенов.",
    ].join("\n"),
    parse_mode: "HTML",
