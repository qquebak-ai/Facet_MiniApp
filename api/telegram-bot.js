...
async function welcome(chatId) {
  ...
  //
  // Первым сообщением идёт баннер: Telegram скачивает его по ссылке сам.
  // Берём почти квадратный вариант: широкий Telegram ужимает по ширине
  // пузыря, и от него остаётся полоска, где ничего не разобрать.
  // Если не скачал (домен ещё не развёрнут, картинка переименована),
  // приветствие всё равно должно дойти — поэтому текстом.
  const ответ = await tgCall("sendPhoto", {
    chat_id: chatId,
    photo: `${APP_URL}/hero-bot.png?v=${БАННЕР_ВЕРСИЯ}`,
    caption: ПРИВЕТСТВИЕ,
    reply_markup: menuButtons(),
  }).catch(() => null);
  if (ответ && ответ.ok) return;
  console.warn("[bot] баннер не ушёл:", ответ && ответ.description);
  await tg("sendMessage", {
    chat_id: chatId,
    text: ПРИВЕТСТВИЕ,
    reply_markup: menuButtons(),
