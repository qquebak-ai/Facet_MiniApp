...
function нарисовать({ Ш, В, кегльНазвания, кегльПодписи, кегльПлашки, листВысота, плиткаКегль, вРяду, доли }) {
  ...
  const центрX = Ш * доли.листX;
  const низЛиста = В * доли.низЛиста;
  ...
  const текстX = Math.round(Ш * доли.текстX);
  const знакY = Math.round(В * доли.знак);
  const плашкаY = Math.round(В * доли.плашка);
  const названиеY = Math.round(В * доли.название);
  const подписьY = Math.round(В * доли.подпись);
  ...
  // Без плиток обходится вариант для чата: там картинку ужимают до
  // ширины пузыря, и подписи в них превращаются в серую рябь.
  const плитки = (доли.плитки ? ПЛИТКИ.slice(0, вРяду * 2) : [])
    .map((п, i) => {
...
${п.строки
  ...
  <text x="${текстX}" y="${подписьY + Math.round(кегльПодписи * 2.5)}" fill="${Ц.основной}" font-family="${шрифт}" font-size="${кегльПодписи}" font-weight="700" letter-spacing="0.02em">ЗА ПАРУ СЕКУНД</text>
...
const размеры = [
  {
    ...
    доли: { текстX: 0.042, знак: 0.095, плашка: 0.215, название: 0.375, подпись: 0.565, плитки: 0.705, листX: 0.7, низЛиста: 0.715 },
  },
  {
  ...
  {
    // Для чата: почти квадрат. Широкую картинку Telegram ужимает по
    // ширине пузыря, и от неё остаётся полоска, где не разобрать ни
    // подписи, ни лист.
    имя: "hero-bot", Ш: 1200, В: 900,
    кегльНазвания: 104, кегльПодписи: 34, кегльПлашки: 24,
    листВысота: 530, плиткаКегль: 20, вРяду: 3,
    доли: { текстX: 0.055, знак: 0.07, плашка: 0.175, название: 0.32, подпись: 0.56, плитки: 0, листX: 0.72, низЛиста: 0.755 },
  },
];
