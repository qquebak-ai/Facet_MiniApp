...
const ПЛИТКИ = [
  { верх: "Запуск токена", низ: "за минуту", икона: "M 13.5 2 L 5 13.5 h 5.5 L 10 22 l 8.5 -11.5 H 13 z" },
  { верх: "Бондинг-кривая", низ: "без пресейла", икона: "M 3 21 h 18 M 4 18 C 9 18, 13 13, 16 5 M 16 5 l -4.4 1.2 M 16 5 l 1.2 4.4" },
  { верх: "Комиссия", низ: "1%", икона: "M 6 6 a 2.4 2.4 0 1 0 0.01 0 M 18 18 a 2.4 2.4 0 1 0 0.01 0 M 19 5 L 5 19" },
  { верх: "Открытый", низ: "контракт", икона: "M 12 3 l 7 3 v 6 c 0 4.4 -3 7.6 -7 9 c -4 -1.4 -7 -4.6 -7 -9 V 6 z" },
  { верх: "TON", низ: "и Solana", икона: "M 4 8 h 13 M 13 4 l 4 4 l -4 4 M 20 16 H 7 M 11 12 l -4 4 l 4 4" },
  { верх: "Листинг", низ: "на 85 TON", икона: "M 5 20 V 12 M 12 20 V 5 M 19 20 v -5 M 3 20 h 18" },
];
...
function нарисовать({ Ш, В, кегльНазвания, кегльПодписи, кегльПлашки, листВысота, плиткаКегль, вРяду }) {
  ...
  const центрX = Ш * 0.7;
  const низЛиста = В * 0.79;
  const полY = низЛиста + листВысота * 0.02;
  ...
  const плиткаВ = Math.round(плиткаКегль * 4.4);
  ...
  const плитки = ПЛИТКИ.slice(0, вРяду === 3 ? 6 : вРяду * 2)
    ...
    <text x="${тx}" y="${(плиткаВ / 2 - плиткаКегль * 0.12).toFixed(0)}" fill="${Ц.белый}" font-family="${шрифт}"
      font-size="${плиткаКегль}" font-weight="600">${п.верх}</text>
    <text x="${тx}" y="${(плиткаВ / 2 + плиткаКегль * 1.15).toFixed(0)}" fill="${Ц.тусклый}" font-family="${шрифт}"
      font-size="${плиткаКегль}" font-weight="500">${п.низ}</text>
  </g>`;
    })
    .join("\n");
