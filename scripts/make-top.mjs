...
/* Парящая монета для фона шапки.
 *
 * Листья на тёмном фоне превращались в серые пятна: плоская заливка и
 * тёмные жилки на таком же тёмном фоне. Сплюснутые кружки вышли не
 * лучше — читались блинами. Монету делает монетой ободок: светлое ребро
 * по краю, тёмное поле внутри и знак сети на нём. Тогда предмет узнаётся
 * даже размером с ноготь.
 *
 * Наклон лёгкий — сжатие по вертикали не больше десятой доли: сильнее и
 * круг перестаёт быть кругом. */
function монета(x, y, r, поворот, вид, прозрачность) {
  const знак = вид === "тон"
    ? `<g transform="translate(${-r * 0.42}, ${-r * 0.42}) scale(${(r * 0.0155).toFixed(4)})" fill="#EAF6FF" fill-opacity="0.92">
    ...
    : вид === "сол"
      ? `<g transform="translate(${-r * 0.4}, ${-r * 0.3}) scale(${(r * 0.002).toFixed(4)})" fill="url(#солГрад)">
      ...
      : "";
  const сжатие = 0.93;
  return `<g transform="translate(${x}, ${y}) rotate(${поворот})" opacity="${прозрачность}">
    <circle r="${r * 1.7}" fill="url(#свечение-${вид})"/>
    <ellipse rx="${r}" ry="${r * сжатие}" fill="url(#ребро-${вид})"/>
    <ellipse rx="${r * 0.82}" ry="${r * сжатие * 0.82}" fill="url(#поле-${вид})"/>
    ${знак}
    <ellipse cx="${-r * 0.3}" cy="${-r * 0.42}" rx="${r * 0.34}" ry="${r * 0.16}" fill="#ffffff" opacity="0.22" transform="rotate(-24)"/>
  </g>`;
}
...
async function нарисовать(строки) {
  ...
  ${[
    // Монеты в правом углу шапки: разные размеры, наклоны и глубина —
    // ближние крупнее и ярче, дальние глуше. Левее 700 не заходим: там
    // текст.
    [896, 122, 48, -12, "тон", 1],
    [1012, 226, 34, 14, "сол", 0.9],
    [790, 210, 22, -20, "тьма", 0.6],
    [948, 330, 27, 8, "сол", 0.55],
    [1048, 100, 18, 22, "тьма", 0.45],
    [852, 384, 21, -8, "тон", 0.5],
  ].map((м) => монета(...м)).join("\n  ")}
