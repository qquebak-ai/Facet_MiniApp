 ...
 * Выход:   public/start.svg + .png — 1200×820
 */
...
/* Знак: буква M, выложенная листьями.
 *
 * Не «штрих = лист», а мозаика: вдоль каждого штриха буквы в несколько
 * рядов лежат настоящие листья — с зубчатым краем и жилками, под
 * разными углами и в разных оттенках, слегка перекрывая друг друга.
 * Форма листа берётся из приложения (LEAF_KINDS[2]), чтобы знак и
 * интерфейс были из одного мира.
 *
 * Раскладка детерминированная: одно зерно — одна и та же картинка,
 * иначе её нельзя ни поправить, ни сравнить с прошлой.
 */
...
/* Оттенки листвы: от глубокого до светлого. Один цвет на всю букву
   превращает её в пятно — рельеф читается именно за счёт разницы. */
const ОТТЕНКИ = ["#4A52C8", "#5A64E8", "#6C7CFF", "#8A93FF", "#A3AAFF"];

/* Штрихи буквы M в квадрате 100×100: отрезок и его толщина. */
const ШТРИХИ = [
  { от: [18, 92], до: [18, 16], толщина: 15 },
  { от: [82, 92], до: [82, 16], толщина: 15 },
  { от: [20, 18], до: [50, 66], толщина: 14 },
  { от: [80, 18], до: [50, 66], толщина: 14 },
];

function случай(зерно) {
  let a = зерно >>> 0;
  return () => {
    a = (a + 0x6D2B79F5) >>> 0;
...
function знакИзЛистьев() {
  ...
  const сл = случай(20260905);
  const листья = [];

  for (const ш of ШТРИХИ) {
    const dx = ш.до[0] - ш.от[0];
    const dy = ш.до[1] - ш.от[1];
    const длина = Math.hypot(dx, dy);
    const вдольX = dx / длина;
    const вдольY = dy / длина;
    // Поперёк штриха — перпендикуляр: по нему листья расходятся в ряды.
    const попёрX = -вдольY;
    const попёрY = вдольX;
    const уголШтриха = (Math.atan2(dx, -dy) * 180) / Math.PI;

    // Листья крупные и редкие: частая мелкая листва сливалась в мех, и
    // буква читалась пятном. Два ряда вместо трёх, шаг вдвое шире.
    const шаг = 13;
    for (let t = 4; t <= длина - 3; t += шаг) {
      for (const ряд of [-1, 1]) {
        const сдвиг = ряд * ш.толщина * 0.26 + (сл() - 0.5) * 2;
        const вдоль = t + (сл() - 0.5) * 3;
        листья.push({
          x: ш.от[0] + вдольX * вдоль + попёрX * сдвиг,
          y: ш.от[1] + вдольY * вдоль + попёрY * сдвиг,
          угол: уголШтриха + ряд * 22 + (сл() - 0.5) * 24,
          размер: 24 + сл() * 6,
          цвет: ОТТЕНКИ[Math.floor(сл() * ОТТЕНКИ.length)],
        });
      }
    }
  }

  // Рисуем сверху вниз: нижние листья ложатся поверх верхних, и край
  // буквы выглядит собранным, а не осыпающимся.
  листья.sort((a, b) => a.y - b.y);

  return листья.map((л) => {
    const s = л.размер / 30;   // контур листа высотой около 30 своих единиц
    return `<g transform="translate(${л.x.toFixed(2)}, ${л.y.toFixed(2)}) rotate(${л.угол.toFixed(1)}) scale(${s.toFixed(3)}) translate(0, 14)">
      <path d="${лист.outline}" fill="${л.цвет}"/>
      <g fill="none" stroke="#2A2E5C" stroke-opacity="0.55" stroke-width="0.5" stroke-linecap="round">
