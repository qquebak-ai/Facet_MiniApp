...
const DARK_THEME = {
  bg: "#0D1117",
  surface: "rgba(23,29,38,0.60)",
  surfaceHi: "rgba(32,40,51,0.64)",
  line: "rgba(255,145,102,0.12)",
  lineHi: "rgba(255,145,102,0.26)",
  ice: "#F3F3F6",
  muted: "#8A93A0",
  electric: "#FF6B35",
  turquoise: "#FF6B35",
...
/* White theme is a distinct look, not a flat color-invert of Dark:
   ...
   near-black ink for text, and the same Ember accent as the dark
   theme (instead of a separate color) so the brand palette stays
   recognizable across both modes. */
const WHITE_THEME = {
  bg: "#F5F5F7",
  surface: "rgba(255,255,255,0.72)",
  surfaceHi: "rgba(240,239,244,0.75)",
  line: "rgba(255,107,53,0.14)",
  lineHi: "rgba(255,107,53,0.28)",
  ice: "#131316",
  muted: "#75757F",
  electric: "#FF6B35",
  turquoise: "#FF6B35",
...
function applyTheme(mode) {
  ...
  PRISM_TEXT = mode === "White" ? "#FFFFFF" : "#0D1117";
}
...
const DARK_PRISM = `linear-gradient(135deg, #FF6B35 0%, #E8501F 100%)`;
const LIGHT_PRISM = `linear-gradient(135deg, #FF6B35 0%, #E8501F 100%)`;
let PRISM = DARK_PRISM;
let PRISM_TEXT = "#0D1117"; // Midnight text reads best on the Ember gradient; White theme uses the same warm gradient, so it keeps white text for contrast
...
function CyberGrid({ forceDark }) {
  const lineC = forceDark ? "rgba(255,255,255,0.05)" : glow(0.06);
  const strokeC = forceDark ? "#FFFFFF" : T.turquoise;
    ...
    <div className="fx-view flex flex-col gap-4 pb-4" style={{ position: "relative" }}>
      ...
      {tab === "chart" && (
        <>
          ...
          <div className="rounded-2xl overflow-hidden" style={{ background: T.surface, border: `1px solid ${T.line}`, padding: 2 }}>
...
function TradeModal({ t: token, tradeModal, onClose, onConfirm, walletTonBalance = 0, tonPriceUsd = 0, heldAmount = 0 }) {
  ...
  return (
    <div className="fx-modal-back" style={{ position: "absolute", inset: 0, zIndex: 60, background: "rgba(9,12,17,0.75)", backdropFilter: "blur(4px)", display: "flex", alignItems: "flex-end" }} onClick={onClose}>
...
function ImageCropModal({ file, shape = "circle", onCancel, onConfirm }) {
  ...
  return (
    <div className="fx-modal-back" style={{ position: "absolute", inset: 0, zIndex: 90, background: "rgba(9,12,17,0.9)", backdropFilter: "blur(6px)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }} onClick={(e) => { e.stopPropagation(); onCancel(); }}>
      <div className="fx-modal-card" onClick={(e) => e.stopPropagation()} style={{ width: "100%", maxWidth: 340, background: T.surface, border: `1px solid ${T.lineHi}`, borderRadius: 24, padding: 20, display: "flex", flexDirection: "column", alignItems: "center", gap: 16 }}>
        ...
        <div className="flex items-center gap-2 w-full">
          <button onClick={onCancel} className="fx-tap flex-1 rounded-xl py-2.5" style={{ background: "transparent", border: `1px solid ${T.line}`, fontFamily: bodyFont, fontSize: 13, color: T.muted }}>{t("cancel")}</button>
          <button onClick={handleConfirm} className="fx-tap flex-1 rounded-xl py-2.5" style={{ background: PRISM, color: PRISM_TEXT, fontFamily: displayFont, fontWeight: 700, fontSize: 13 }}>{t("cropConfirm")}</button>
        </div>
      </div>
    </div>
  );
...
function TokenLaunchOverlay({ open, form, category, logoUrl, buyAmount, stepIndex, done, error, result, onClose, onRetry, onViewToken }) {
  ...
  return (
    <div className="fx-modal-back" style={{ position: "absolute", inset: 0, zIndex: 80, background: "rgba(9,12,17,0.92)", backdropFilter: "blur(6px)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      {error ? (
...
function PinSetupModal({ mode, currentPin, onClose, onComplete, onDisable, showToast }) {
  ...
  return (
    <div className="fx-modal-back" style={{ position: "absolute", inset: 0, zIndex: 70, background: "rgba(9,12,17,0.82)", backdropFilter: "blur(4px)", display: "flex", alignItems: "flex-end" }} onClick={onClose}>
...
function PinLockScreen({ pin, profile, onUnlock, onForgot }) {
  ...
  return (
    <div className="fx-view" style={{ position: "absolute", inset: 0, zIndex: 200, background: "#0D1117", display: "flex", flexDirection: "column", alignItems: "center", overflow: "hidden" }}>
...
function ProfileView({
  ...
  return (
    <div className="fx-view" style={{ position: "relative" }}>
      ...
      {deleteConfirmOpen && (
        <div className="fx-modal-back" style={{ position: "fixed", inset: 0, zIndex: 70, background: "rgba(9,12,17,0.8)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }} onClick={() => !deleting && setDeleteConfirmOpen(false)}>
