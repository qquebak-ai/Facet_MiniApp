  ...
  return (
    <style>{`
      ...
      * { -webkit-tap-highlight-color: transparent; }
      ...
      @keyframes shineSweep { 0%{ transform:translateX(-40%); } 50%{ transform:translateX(340%); } 100%{ transform:translateX(340%); } }
      .glass-widget { position: relative; overflow: hidden; backdrop-filter: blur(24px) saturate(160%); -webkit-backdrop-filter: blur(24px) saturate(160%); }
      .glass-widget::before { content: ""; position: absolute; inset: 0; border-radius: inherit; background: radial-gradient(90px 50px at 16% 0%, rgba(255,255,255,0.30), transparent 60%); pointer-events: none; }
      .glass-widget::after { content: ""; position: absolute; top: 0; left: -30%; width: 34%; height: 100%; background: linear-gradient(100deg, transparent, rgba(255,255,255,0.14), transparent); animation: shineSweep 6.5s ease-in-out infinite; pointer-events: none; }
    ...
    <div style={{ background: T.bg, height, minHeight: height, width: "100%", maxWidth: 480, margin: "0 auto", fontFamily: bodyFont, position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ position: "relative", zIndex: 1, height: "100%", display: "flex", flexDirection: "column" }}>
        <div className="px-4 pt-4 pb-2" style={{ flexShrink: 0 }}>
          <div
            className="glass-widget flex items-center justify-between"
            style={{
              padding: "10px 12px 10px 10px",
              borderRadius: 24,
              background: `linear-gradient(155deg, ${ink(0.14)}, ${ink(0.04)} 55%, ${ink(0.08)})`,
              border: `1px solid ${ink(0.30)}`,
              boxShadow: `0 16px 34px -12px rgba(0,0,0,0.45), inset 0 1px 0 ${ink(0.45)}`,
            }}
          >
            <button onClick={() => goTab("home")} className="fx-tap flex items-center gap-2" style={{ position: "relative", zIndex: 1 }}>
              <div style={{ width: 32, height: 32, borderRadius: 10, background: "linear-gradient(155deg, #FF8A5C, #E85A2A)", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: `0 4px 12px ${hexA("#FF6B35", 0.4)}, inset 0 1px 0 rgba(255,255,255,0.5)`, flexShrink: 0 }}>
                <svg width="15" height="15" viewBox="0 0 24 24" fill="none"><path d="M4 17V7l8 6 8-6v10" stroke="#fff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></svg>
              </div>
              <span style={{ fontFamily: logoFont, color: T.ice, fontSize: 24, fontWeight: 600, fontStyle: "italic", letterSpacing: "0em" }}>Mintly</span>
            </button>
            <button onClick={handleHeaderWalletClick} className="fx-tap flex items-center gap-1.5 rounded-full px-3 py-1.5" style={{ position: "relative", zIndex: 1, background: ink(0.10), border: `1px solid ${connected ? glow(0.4) : ink(0.30)}`, boxShadow: `inset 0 1px 0 ${ink(0.30)}` }}>
