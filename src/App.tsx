    ...
    <div className="fx-modal-back" style={{ position: "absolute", inset: 0, zIndex: 60, background: "rgba(0,0,0,0.75)", backdropFilter: "blur(4px)", display: "flex", alignItems: "flex-end" }} onClick={onClose}>
      <div className="fx-modal-card" onClick={(e) => e.stopPropagation()} style={{ width: "100%", background: T.surface, border: `1px solid ${T.lineHi}`, borderRadius: "22px 22px 0 0", padding: 22 }}>
        ...
        <div className="flex flex-col gap-2">
          ...
          {/* Local-only tokens (not yet backed by Supabase) can be removed
             from this device's list — useful for clearing out testnet
             launches. Only shows for tokens created through this app's
             own launch flow (they carry an `id` starting "local-"), not
             for the static MY_TOKENS demo entries. */}
          {onDelete && String(token.id || "").startsWith("local-") && (
            confirmingDelete ? (
              <div className="flex gap-2">
                <button
    ...
    <div className="fx-view" style={{ position: "relative" }}>
      <div className="flex flex-col gap-0 pb-4">
        ...
        <div className="mt-5">
          <SectionTitle action={
            <div className="flex items-center gap-3">
              {onClearAllTokens && myTokens.length > 0 && (
                confirmingClearAll ? (
                  <div className="flex items-center gap-1.5">
...
const FEE_PERCENT = 0.01; // 1% комиссии
  ...
  async function deleteMyToken(id) {
    ...
    if (error) { console.error("[mintly] failed to delete token from Supabase:", error); showToast(t("deleteFailedToast")); if (userId) loadMyTokens(userId); }
  }
  async function clearAllMyTokens() {
    const ids = myTokens.map((tok) => tok.id);
    setMyTokens([]);
    setHoldings({});
    try { if (typeof window !== "undefined") window.localStorage.removeItem("mintly_holdings"); } catch (e) { /* localStorage unavailable */ }
    if (!ids.length) return;
    const { error } = await supabase.from("tokens").delete().in("id", ids);
    if (error) { console.error("[mintly] failed to clear tokens in Supabase:", error); showToast(t("deleteFailedToast")); if (userId) loadMyTokens(userId); }
  }
