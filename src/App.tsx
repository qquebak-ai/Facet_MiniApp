        ...
        {chartData && (() => {
          ...
          return (
            <div className="flex items-center gap-2.5 flex-wrap" style={{ fontFamily: monoFont, fontSize: 10, color: T.muted, marginTop: 5, marginBottom: 6 }}>
              <span>О <span style={{ color: T.ice }}>{fmtPrice(ref.open)}</span></span>
              <span>В <span style={{ color: T.ice }}>{fmtPrice(ref.high)}</span></span>
              <span>Н <span style={{ color: T.ice }}>{fmtPrice(ref.low)}</span></span>
              <span>З <span style={{ color: T.ice }}>{fmtPrice(ref.close)}</span></span>
      ...
      <div className="fx-modal-card" onClick={(e) => e.stopPropagation()} style={{ width: "100%", background: T.surface, border: `1px solid ${T.lineHi}`, borderRadius: "22px 22px 0 0", padding: 20, maxHeight: "88%", overflowY: "auto" }}>
        ...
        <div className="flex items-center justify-between" style={{ marginTop: 16 }}>
          ...
          <span style={{ fontFamily: bodyFont, color: T.muted, fontSize: 11 }}>
            Доступно: {isBuy ? `$${availableUSD.toFixed(2)}` : `${holdingTokens.toLocaleString("ru-RU")} ${token.ticker}`}
          </span>
        </div>
        ...
        <div className="flex flex-col gap-1.5" style={{ marginTop: 14, fontFamily: monoFont, fontSize: 11, color: T.muted }}>
          <div className="flex justify-between"><span>Курс</span><span style={{ color: T.ice }}>{fmtPrice(token.price)} / {token.ticker}</span></div>
          ...
          <div className="flex justify-between"><span>Мин. получите (с учётом slippage)</span><span style={{ color: T.ice }}>{amount > 0 ? (isBuy ? `${(estimate * (1 - slippage / 100)).toLocaleString("ru-RU", { maximumFractionDigits: 0 })} ${token.ticker}` : `$${(estimate * (1 - slippage / 100)).toFixed(2)}`) : "—"}</span></div>
        </div>
        ...
        <button onClick={handleConfirm} disabled={!canConfirm} className="fx-tap w-full rounded-xl py-3 mt-5" style={{
          ...
          {amount > 0 ? (isBuy ? `Купить за $${amount.toFixed(2)}` : `Продать ${amount.toLocaleString("ru-RU")} ${token.ticker}`) : (isBuy && tonPriceUsd <= 0 ? "Загрузка курса…" : !isBuy && holdingTokens <= 0 ? "Нечего продавать" : "Введите сумму")}
        </button>
      </div>
    </div>
  );
...
function CreateView({ showToast, unlocked, accountCreated, connected, onOpenCreateProfile, onOpenConnectModal, onLaunch }) {
  ...
  function handleLaunch() {
    setTouched(true);
    if (!logoUrl) {
      showToast("Загрузи логотип токена — это обязательно");
      return;
    }
    ...
    if (!form.desc.trim()) {
      showToast("Опиши токен — после запуска описание изменить будет нельзя");
      return;
    }
    const buyNum = parseFloat(form.buyAmount.replace(",", "."));
    if (!Number.isFinite(buyNum) || buyNum <= 0) {
      showToast("Укажи сумму для запуска — на неё будут выкуплены первые токены");
      return;
    }
  ...
  function finishLaunch(result) {
    if (result) showToast(`Токен ${result.name} ($${result.ticker}) создан ✅`);
    resetForm();
  }
  ...
  return (
    <div className="fx-view flex flex-col gap-7 pb-36" style={{ position: "relative" }}>
      ...
      <div>
        <span style={{ fontFamily: bodyFont, color: T.muted, fontSize: 12 }}>Логотип *</span>
        <div className="flex gap-3 mt-1.5">
          ...
          <button onClick={() => logoInputRef.current && logoInputRef.current.click()} className="fx-tap flex flex-col items-center justify-center gap-1 flex-shrink-0 overflow-hidden" style={{ width: 76, height: 76, borderRadius: "50%", background: logoUrl ? `center/cover no-repeat url(${logoUrl})` : T.surface, border: `1px dashed ${touched && !logoUrl ? T.down : T.line}` }}>
            {!logoUrl && (<><ImageIcon size={18} color={touched ? T.down : T.muted} /><span style={{ fontFamily: bodyFont, color: touched ? T.down : T.muted, fontSize: 9 }}>Лого</span></>)}
          </button>
          <button onClick={() => bannerInputRef.current && bannerInputRef.current.click()} className="fx-tap flex-1 flex flex-col items-center justify-center gap-1 rounded-2xl overflow-hidden" style={{ background: bannerUrl ? `center/cover no-repeat url(${bannerUrl})` : T.surface, border: `1px dashed ${T.line}` }}>
            {!bannerUrl && (<><Upload size={18} color={T.muted} /><span style={{ fontFamily: bodyFont, color: T.muted, fontSize: 10 }}>Баннер 1200×400 (необязательно)</span></>)}
          </button>
        </div>
        {touched && !logoUrl && <span style={{ fontFamily: bodyFont, color: T.down, fontSize: 11, marginTop: 4, display: "block" }}>Логотип обязателен</span>}
      </div>

      <Field label="Название" placeholder="Prism Cat" value={form.name} onChange={set("name")} />
      <Field label="Тикет" placeholder="PRSM" value={form.ticker} onChange={set("ticker")} />

      <div>
        <Field label="Описание *" placeholder="О чём этот токен и почему он появился" area value={form.desc} onChange={set("desc")} error={touched && !form.desc.trim()} />
        {touched && !form.desc.trim() && <span style={{ fontFamily: bodyFont, color: T.down, fontSize: 11, marginTop: 4, display: "block" }}>Описание обязательно — после запуска изменить его будет нельзя</span>}
      </div>
      ...
      <div className="flex flex-col gap-1.5">
        <span style={{ fontFamily: bodyFont, color: T.muted, fontSize: 12 }}>Сумма для запуска (TON) *</span>
        ...
        {(() => {
          ...
          if (!Number.isFinite(buyNum) || buyNum <= 0) {
            return (
              <p style={{ fontFamily: bodyFont, color: T.muted, fontSize: 11, lineHeight: 1.5 }}>
                На эту сумму сразу после запуска будут выкуплены первые токены — это стартовая ликвидность и первая цена токена.
              </p>
            );
          }
          ...
          return (
            <div className="flex items-center justify-between rounded-xl px-3.5 py-2.5" style={{ background: ink(0.06), border: `1px solid ${ink(0.2)}` }}>
              <span style={{ fontFamily: bodyFont, color: T.electric, fontSize: 12 }}>Ты получишь ≈</span>
              <span style={{ fontFamily: monoFont, color: T.electric, fontSize: 12.5, fontWeight: 600 }}>
                {tokens.toLocaleString("ru-RU")} {(form.ticker.trim() || "TOKEN").toUpperCase()} · {pct.toFixed(pct < 1 ? 3 : 1)}% выпуска
              </span>
            </div>
          );
        })()}
...
const FEE_PERCENT = 0.01; // 1% комиссии
  ...
  async function confirmTrade(mode, payAmount, receiveAmount, unit, rawAmount, rawEstimate) {
    if (mode === "buy") {
      ...
      if (!(tonPriceUsd > 0)) { showToast("Курс TON ещё загружается, попробуй через секунду"); return; }
      const totalTon = rawAmount / tonPriceUsd;
      const spendableTon = Math.max(0, tonBalance - NETWORK_FEE_TON);
      if (totalTon > spendableTon) { showToast("Недостаточно TON на кошельке для этой суммы"); return; }
      ...
      if (rawAmount > held) { showToast("Недостаточно токенов для продажи этой суммы"); return; }
      if (!connected) { showToast("Подключи TON-кошелёк, чтобы продать"); return; }
