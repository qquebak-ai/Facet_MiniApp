...
import {
  Search, Flame, TrendingUp, TrendingDown, Clock, Sparkles, ArrowUpRight, ArrowDownRight,
...
const STR = {
  RU: {
    ...
    disconnectShort: "Отключить", tonExplorerBtn: "Обозреватель TON", walletProvider: "Кошелёк · Tonkeeper",
    ...
    mempadFilterNew: "Новое", mempadFilterHot: "Горячее", mempadFilterDex: "DEX",
    ...
    txPerHourShort: "тр./ч",
    statTransactions1h: "Транзакции (1ч)",
  ...
  EN: {
    ...
    disconnectShort: "Disconnect", tonExplorerBtn: "TON Explorer", walletProvider: "Wallet · Tonkeeper",
    ...
    mempadFilterNew: "New", mempadFilterHot: "Hot", mempadFilterDex: "DEX",
    ...
    txPerHourShort: "tx/h",
    statTransactions1h: "Transactions (1h)",
...
const logoFont = "'Cormorant Garamond', 'Georgia', serif"; // wordmark only
const bodyFont = "'Inter', -apple-system, 'SF Pro Display', sans-serif";
...
function GlobalStyle() {
  return (
    <style>{`
      @import url('https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,400;0,500;0,600;0,700;1,500&family=Inter:wght@400;500;600;700&family=IBM+Plex+Mono:wght@400;500&display=swap');
      ...
      * { -webkit-tap-highlight-color: transparent; }
      ...
      @keyframes iconFall { 0%{ transform:translateY(0) scale(0.8) rotate(-6deg); opacity:0; } 18%{ opacity:0.85; } 100%{ transform:translateY(70px) scale(1) rotate(4deg); opacity:0; } }
...
/* Kept as a component (rather than deleted at call sites) so nothing else
   in the tree needs to change — it now renders nothing. An editorial,
   premium background is a flat, quiet one: the brief calls for confidence
   through typography and spacing, not decorative texture. */
...
// Set by fetchTonMemePools on every attempt (success clears it, failure
// records why) so the UI can show the *actual* reason the feed is empty
// instead of a generic "nothing here" message — critical since this runs
// inside a Telegram WebView where there's no way to open devtools.
let lastFeedErrorMsg = null;

// Every GeckoTerminal call goes through this instead of calling
// api.geckoterminal.com directly from the browser. Reason: inside a
// Telegram Mini App's WKWebView (and in plenty of regular mobile
// browsers too), a direct cross-origin fetch() to api.geckoterminal.com
// fails with a bare, undiagnosable "Load failed" — WebKit's generic
// stand-in for "this got blocked before any HTTP response came back",
// most commonly a CORS rejection. Routing it through our own Supabase
// Edge Function turns it into a same-origin-as-far-as-CORS-cares call:
// the *server* talks to GeckoTerminal (no browser CORS involved there
// at all), and the Edge Function's own response carries permissive CORS
// headers back to us. Requires the "geckoterminal" function below to be
// deployed — see /supabase/functions/geckoterminal/index.ts.
async function gtProxyFetch(path) {
  const { data, error } = await supabase.functions.invoke("geckoterminal", { body: { path } });
  if (error) throw new Error(error.message || "geckoterminal proxy error");
  if (data && data.__proxyError) throw new Error(data.__proxyError);
  return data;
}

// Filters out non-meme "blue chip" assets that otherwise show up in
// GeckoTerminal's TON pool data alongside real memecoins — stablecoins
// (USDT/USDC/DAI and friends), TON itself and its wrapped/staking
// derivatives. These are real, legitimately huge-liquidity pools, just
// not what this app's feed is for. A high market-cap cutoff is a second,
// broader net for anything not caught by name/ticker (memecoins on TON
// essentially never sustain nine-figure market caps the way established
// assets do).
const NON_MEME_TICKERS = new Set([
  "TON", "WTON", "PTON", "STON",
  "USDT", "USD₮", "USD₮0", "USDC", "DAI", "BUSD", "TUSD", "FDUSD",
  "JUSDT", "JUSDC", "STTON", "TSTON", "HTON", "TONUSDT",
]);
function isMemeToken(ticker, name, mcapNum) {
  const up = (ticker || "").toUpperCase();
  if (NON_MEME_TICKERS.has(up)) return false;
  if (up.includes("USD")) return false; // catches other USD-pegged stablecoin variants
  if ((name || "").toUpperCase().includes("TETHER")) return false;
  if (mcapNum > 150_000_000) return false; // blue-chip-scale cap, not a memecoin
  return true;
}

function shapePoolRows(rows, included, limit) {
  ...
  return rows.slice(0, limit).map((row) => {
    ...
    const txns24h = a.transactions?.h24;
    const txns1h = a.transactions?.h1;
    return {
      id: row.id,
      ...
      volNum,
      // "holders" isn't in GeckoTerminal's pool data at all — the real count
      // comes from tonapi.io's jetton endpoint and is fetched lazily per
      // token when its detail screen opens (see fetchJettonHoldersCount),
      // not here, so we never show an invented number for it. txCount1h/24h
      // ARE real, straight from GeckoTerminal's own per-pool tx stats.
      holders: null,
      txCount1h: txns1h ? txns1h.buys + txns1h.sells : 0,
      txCount24h: txns24h ? txns24h.buys + txns24h.sells : 0,
      cat: "Мемы",
      seed: hashSeed(row.id),
      verified: !!(dex.name || dexId),
      live: true,
    ...
    // Only pools that are genuinely live on a DEX: a real on-chain pool
    // address with a positive traded price. (Deliberately NOT also
    // requiring liqNum > 0 here — GeckoTerminal sometimes omits
    // reserve_in_usd on individual rows, and gating the whole feed on it
    // would wipe out every token whenever that happens. Liquidity is
    // still used below, just to rank/filter within the "DEX" tab.)
  }).filter(t => t.poolAddress && t.price > 0 && isMemeToken(t.ticker, t.name, t.mcapNum));
}

async function fetchJson(path) {
  return gtProxyFetch(path);
}

async function fetchTonMemePools(limit = 18) {
  // Primary source: trending_pools (ranked by GeckoTerminal's own trending
  // score). If that comes back with zero usable rows — which does happen —
  // fall back to the plain top-pools-by-volume endpoint for the same
  // network, so the feed still has real tokens instead of sitting empty.
  try {
    const json = await fetchJson(`/networks/${GT_NETWORK}/trending_pools?page=1&include=base_token,dex`);
    const shaped = shapePoolRows(json?.data || [], json?.included || [], limit);
    if (shaped.length) { lastFeedErrorMsg = null; return shaped; }
  } catch (err) {
    lastFeedErrorMsg = (err && err.message) || String(err);
  }
  try {
    const json = await fetchJson(`/networks/${GT_NETWORK}/pools?page=1&sort=h24_volume_usd_desc&include=base_token,dex`);
    const shaped = shapePoolRows(json?.data || [], json?.included || [], limit);
    if (shaped.length) { lastFeedErrorMsg = null; return shaped; }
    lastFeedErrorMsg = lastFeedErrorMsg || "GeckoTerminal вернул пустой список пулов";
  } catch (err) {
    lastFeedErrorMsg = (err && err.message) || String(err);
  }
...
async function fetchTokenInfo(tokenAddress) {
  ...
  try {
    const json = await gtProxyFetch(`/networks/${GT_NETWORK}/tokens/${tokenAddress}/info`);
...
// Real holder count — GeckoTerminal's pool/token data doesn't include this
// at all, so it's a separate call to tonapi.io's own jetton endpoint, which
// returns an honest `holders_count` straight from its on-chain index. This
// is always the mainnet host: the memecoin feed itself is real mainnet
// tokens regardless of whether the connected wallet is on testnet for
// launch-testing purposes. Cached + lazy per token address, same reasoning
// as fetchTokenInfo above — never called for the whole scrolling list.
const jettonHoldersCache = new Map(); // tokenAddress -> count | null
async function fetchJettonHoldersCount(tokenAddress) {
  if (!tokenAddress) return null;
  if (jettonHoldersCache.has(tokenAddress)) return jettonHoldersCache.get(tokenAddress);
  try {
    const res = await fetch(`https://tonapi.io/v2/jettons/${tokenAddress}`);
    if (!res.ok) { jettonHoldersCache.set(tokenAddress, null); return null; }
    const body = await res.json().catch(() => null);
    const count = typeof body?.holders_count === "number" ? body.holders_count : null;
    jettonHoldersCache.set(tokenAddress, count);
    return count;
  } catch (err) {
    jettonHoldersCache.set(tokenAddress, null);
    return null;
  }
}
...
async function fetchPoolTrades(poolAddress, limit = 25) {
  ...
  try {
    const json = await gtProxyFetch(`/networks/${GT_NETWORK}/pools/${poolAddress}/trades`);
...
async function fetchPoolOHLCV(poolAddress, tf) {
  ...
  const path = `/networks/${GT_NETWORK}/pools/${poolAddress}/ohlcv/${cfg.timeframe}?aggregate=${cfg.aggregate}&limit=${fetchLimit}&currency=usd&token=base`;
  try {
    const json = await gtProxyFetch(path);
...
function TrendFX({ up, seedKey = 1 }) {
  const items = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      out.push({ left: 6 + rand() * 84, delay: rand() * 2.4, dur: 1.9 + rand() * 1.6, size: 12 + rand() * 7 });
    }
    return out;
  }, [up, seedKey]);
  ...
  return (
    <div style={{
      ...
      /* мягкая маска по верхнему/нижнему краю: иконки затухают в прозрачность
      ...
      {up
        ...
        : items.map((it, i) => (
            <TrendingDown
              ...
              size={it.size}
              style={{
                position: "absolute", left: `${it.left}%`, top: "-16%",
                color: T.down,
                filter: `drop-shadow(0 0 4px ${hexA(T.down, 0.5)})`,
                animation: `iconFall ${it.dur}s cubic-bezier(0.3,0.1,0.4,1) ${it.delay}s infinite`,
              }}
            />
          ))}
    </div>
...
function TokenAvatar({ children, size = 52, tone = "neutral", src }) {
  ...
  const ringColor = tone === "up" ? T.up : tone === "down" ? T.down : T.lineHi;
  return (
    <div
      className="fx-avatar"
      style={{
...
function TokenCard({ t, onOpen, index }) {
  const up = t.change >= 0;
  return (
    <button onClick={() => onOpen(t)} className="fx-card w-full text-left rounded-[22px]" style={{ background: T.surface, border: `1px solid ${up ? "rgba(49,208,123,0.28)" : "rgba(255,77,77,0.24)"}`, padding: "9px 12px", animationDelay: `${index * 55}ms`, position: "relative", overflow: "hidden" }}>
      ...
      <div className="flex items-center gap-3 mt-2 pt-2" style={{ borderTop: `1px solid ${T.line}` }}>
        <CardStat icon={Clock}>{(t.txCount1h || 0).toLocaleString("ru-RU")} {tr("txPerHourShort")}</CardStat>
      </div>
    </button>
  );
}
...
function TokenCardSkeleton({ index }) {
  return (
    <div className="fx-card w-full rounded-[22px]" style={{ background: T.surface, border: `1px solid ${T.line}`, padding: "9px 12px", animationDelay: `${index * 55}ms` }}>
...
// GeckoTerminal's free public API caps out around ~10 requests/minute.
// The token feed is the single most important call to keep alive, so it
// gets a conservative cadence here — fast enough to feel live, slow
// enough to leave headroom for the per-card sparklines and the open
// token's chart to also fetch without the whole feed getting 429'd and
// silently going empty.
const TOKEN_REFRESH_MS = 20_000;
...
function MempadRow({ t: tok, onOpen, index }) {
  return (
    <button onClick={() => onOpen(tok)} className="fx-card w-full flex items-center gap-3 rounded-[22px] px-3 py-2.5 text-left" style={{ background: T.surface, border: `1px solid ${T.line}`, animationDelay: `${index * 55}ms` }}>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5" style={{ overflow: "hidden" }}>
          <span style={{ fontFamily: displayFont, color: T.ice, fontSize: 14, fontWeight: 700, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{tok.ticker}</span>
          {tok.dexName && (
            <span style={{ fontFamily: monoFont, color: T.up, fontSize: 9, fontWeight: 600, background: "rgba(56,211,159,0.12)", border: "1px solid rgba(56,211,159,0.3)", borderRadius: 5, padding: "1px 5px", whiteSpace: "nowrap", flexShrink: 0 }}>{tok.dexName}</span>
          )}
        </div>
        <CardStat icon={Clock}>{(tok.txCount1h || 0).toLocaleString("ru-RU")} {tr("txPerHourShort")}</CardStat>
      </div>
      <div className="text-right flex-shrink-0">
        <div style={{ fontFamily: displayFont, color: T.up, fontSize: 13, fontWeight: 700 }}>{fmtUSD(tok.mcapNum)}</div>
        <div style={{ marginTop: 2 }}><ChangeBadge value={tok.change} /></div>
      </div>
    </button>
  );
}
...
function MempadRowSkeleton({ index }) {
  return (
    <div className="fx-card w-full flex items-center gap-3 rounded-[22px] px-3 py-2.5" style={{ background: T.surface, border: `1px solid ${T.line}`, animationDelay: `${index * 55}ms` }}>
...
function MempadView({ tokens, myTokens = [], loading, error, onRetry, onOpen, onLaunch }) {
  ...
  // "New" is intentionally NOT part of the live external GeckoTerminal
  // feed — it only ever shows tokens actually launched through this
  // Mintly app (myTokens), newest first. dex/hot still reflect the real
  // external on-chain feed like before.
  const myTokensNormalized = useMemo(() => {
    return [...myTokens]
      .map((mt) => ({
        ...mt,
        change: mt.change ?? 0,
        holders: mt.holders ?? 0,
        txCount1h: mt.txCount1h ?? 0,
        seed: mt.seed ?? hashSeed(mt.id),
      }))
      .sort((a, b) => (b.createdAtMs || 0) - (a.createdAtMs || 0));
  }, [myTokens]);

  // Every entry in `tokens` already comes from a real, currently-trading
  // on-chain pool (see fetchTonMemePools) — nothing here is invented or
  // demo data. The tabs below re-slice either that same real feed, or
  // (for "new") your own locally-launched tokens:
  //  - dex:  actually listed & liquid on a DEX right now, ranked by real
  //          pooled liquidity (highest first) — the "safest"/most-traded view.
  //  - new:  tokens YOU launched through this app, newest first.
  //  - hot:  ranked by real 24h price change, biggest movers first.
  const list = useMemo(() => {
    if (filter === "new") return myTokensNormalized;
    let arr = tokens.filter(tok => !spotlight || tok.id !== spotlight.id);
    switch (filter) {
      case "hot":
        arr = [...arr].sort((a, b) => b.change - a.change);
        break;
      default: // dex
        arr = arr.filter(tok => tok.verified).sort((a, b) => (b.liqNum || 0) - (a.liqNum || 0));
        break;
    }
    return arr;
  }, [tokens, filter, spotlight, myTokensNormalized]);
  ...
  return (
    <div className="flex flex-col gap-5" style={{ paddingBottom: 12 }}>
      ...
      ) : !loading && tokens.length === 0 && myTokensNormalized.length === 0 ? (
        <div className="fx-card rounded-[22px] p-6 flex flex-col items-center text-center gap-3" style={{ background: T.surface, border: `1px solid ${T.line}` }}>
          <HeartCrack size={28} color={T.muted} />
          <span style={{ fontFamily: bodyFont, color: T.ice, fontSize: 14, fontWeight: 600 }}>Не удалось загрузить токены</span>
          {error && (
            <span style={{ fontFamily: monoFont, color: T.down, fontSize: 11, lineHeight: 1.5, wordBreak: "break-word" }}>{error}</span>
          )}
          <button onClick={onRetry} className="fx-tap flex items-center gap-1.5 rounded-full px-3.5 py-2 mt-1" style={{ background: T.surfaceHi, border: `1px solid ${T.line}` }}>
            <RefreshCw size={13} color={T.ice} />
            <span style={{ fontFamily: bodyFont, color: T.ice, fontSize: 12.5, fontWeight: 600 }}>Повторить</span>
          </button>
        </div>
      ) : spotlight && (
        <div>
          <SectionTitle>{t("mempadSpotlight")}</SectionTitle>
          <button onClick={() => onOpen(spotlight)} className="fx-card w-full flex flex-col items-center text-center gap-2.5 rounded-[22px] p-6" style={{ background: T.surface, border: `1px solid ${T.line}`, position: "relative", overflow: "hidden" }}>
            <TrendFX up={spotlight.change >= 0} seedKey={spotlight.seed} />
            <div style={{ position: "relative", zIndex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
              ...
              <CardStat icon={Clock}>{(spotlight.txCount1h || 0).toLocaleString("ru-RU")} {tr("txPerHourShort")}</CardStat>
      ...
      {(tokens.length > 0 || myTokensNormalized.length > 0) && (
        <>
...
function TokenDetail({ t, onBack, showToast, onBuy, onSell, unlocked = true, connected = true, onConnectWallet, themeKey }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      let result = null;
      if (t.poolAddress) result = await fetchPoolOHLCV(t.poolAddress, tf);
      if (!result) result = genSyntheticCandles(t.price, t.seed, tf, CHART_TOTAL);
      if (!cancelled) { setChartData({ ...result, isLive: !!t.poolAddress }); setChartLoading(false); }
    })();
    return () => { cancelled = true; };
  }, [tf, t.id, t.poolAddress]);
  ...
  useEffect(() => {
    ...
    const iv = setInterval(async () => {
      if (t.poolAddress) {
        const fresh = await fetchPoolOHLCV(t.poolAddress, tf);
  ...
  const supplyEst = t.price > 0 ? t.mcapNum / t.price : 0;
  ...
  function handleShare() {
    const url = `https://mintly.app/token/${t.id}`;
  ...
  const [favCount, bumpFav] = useLocalCounter(`mintly_fav_${t.id}`, true);
  const [hypeCount, bumpHype] = useLocalCounter(`mintly_hype_${t.id}`, true);
  const [rugCount, bumpRug] = useLocalCounter(`mintly_rug_${t.id}`, true);
  function handleRug() {
  ...
  // Real holder count from tonapi.io — null while loading/unavailable, in
  // which case the UI shows "—" rather than a made-up number.
  const [holdersCount, setHoldersCount] = useState(null);
  useEffect(() => {
    let cancelled = false;
    setHoldersCount(null);
    if (t.tokenAddress) {
      fetchJettonHoldersCount(t.tokenAddress).then((res) => { if (!cancelled) setHoldersCount(res); });
    }
    return () => { cancelled = true; };
  }, [t.tokenAddress]);
  ...
  useEffect(() => {
    if (tab !== "tx" || !t.poolAddress) return;
    let cancelled = false;
    setTradesLoading(true);
    async function load() {
      const res = await fetchPoolTrades(t.poolAddress);
    ...
    <div className="fx-view flex flex-col gap-4 pb-4" style={{ position: "relative" }}>
      ...
      <div className="flex items-center justify-between gap-2 flex-wrap">
        <div className="flex items-center gap-3">
          <TokenAvatar size={52} tone={up ? "up" : "down"} src={t.logoUrl}>{t.emoji}</TokenAvatar>
          <div>
            <div className="flex items-center gap-1.5">
              <span style={{ fontFamily: displayFont, color: T.ice, fontSize: 18, fontWeight: 700 }}>{t.name}</span>
              {t.verified && <ShieldCheck size={14} color={T.electric} />}
            </div>
            <span style={{ fontFamily: monoFont, color: T.muted, fontSize: 11.5 }}>
              ${t.ticker} · {fmtAge(t.createdAt) ? `${fmtAge(t.createdAt)} · ` : ""}{t.dexName || catLabel(t.cat)}
            </span>
          </div>
        </div>
        {t.tokenAddress ? (
          <button onClick={copyContract} className="fx-tap flex items-center gap-1.5 rounded-full px-3 py-1.5" style={{ background: T.surface, border: `1px solid ${T.line}` }}>
            <span style={{ fontFamily: monoFont, color: T.ice, fontSize: 12 }}>{shortAddr(t.tokenAddress)}</span>
      ...
      {/* Stats: big Market Cap on the left, compact Holders/Transactions/
          Volume column on the right — same information density as the
          reference layout. */}
      <div className="flex items-start justify-between gap-3">
        <div>
          <div style={{ fontFamily: bodyFont, color: T.muted, fontSize: 11 }}>{tr("marketCapLabel")}</div>
          ...
          <div style={{ fontFamily: monoFont, color: T.muted, fontSize: 12, marginTop: 2 }}>{fmtPrice(t.price)} {tr("perToken")}</div>
        </div>
        <div className="flex flex-col items-end gap-1" style={{ fontFamily: bodyFont, fontSize: 12, paddingTop: 2 }}>
          ...
          <div className="flex items-center gap-2"><span style={{ color: T.muted }}>{tr("statTransactions1h")}</span><span style={{ fontFamily: monoFont, color: T.ice }}>{(t.txCount1h || 0).toLocaleString("ru-RU")}</span></div>
          <div className="flex items-center gap-2"><span style={{ color: T.muted }}>{tr("statVolume24h")}</span><span style={{ fontFamily: monoFont, color: T.ice }}>${t.vol}</span></div>
        </div>
      </div>
      ...
      {tab === "chart" && (
        <>
          <div className="flex items-center justify-between gap-2 flex-wrap">
            <div className="flex gap-1 rounded-[16px] p-0.5" style={{ background: T.surface, border: `1px solid ${T.line}` }}>
              {[["mcap", tr("chartModeMcap")], ["price", tr("chartModePrice")]].map(([id, label]) => (
                <button key={id} onClick={() => setChartMode(id)} className="fx-tap rounded-[14px] px-2.5 py-1" style={{ fontFamily: bodyFont, fontSize: 11.5, fontWeight: 600, background: chartMode === id ? T.ice : "transparent", color: chartMode === id ? T.bg : T.muted }}>{label}</button>
              ))}
            </div>
          ...
          <div className="rounded-[22px] overflow-hidden" style={{ background: T.surface, border: `1px solid ${T.line}`, padding: 2 }}>
            {chartLoading || !chartData ? (
              <div className="flex items-center justify-center" style={{ height: 340, fontFamily: monoFont, fontSize: 11, color: T.muted }}>
                {tr("chartLoading")}
              </div>
            ) : (
              <TerminalChart key={`${t.id}-${tf}-${chartMode}`} candles={scaledCandles} height={340} themeKey={themeKey} onHover={setHovered} tf={tf} valueFmt={chartMode === "price" ? fmtPrice : fmtUSD} />
            )}
          </div>
      ...
      {tab === "info" && (
        <div className="flex flex-col gap-3">
          <div className="grid grid-cols-2 gap-2">
            <StatChip icon={TrendingUp} label={tr("statPrice")} value={fmtPrice(t.price)} />
            <StatChip icon={Wallet} label={tr("statLiquidity")} value={`$${t.liq}`} />
            <StatChip icon={User} label={tr("statHolders")} value={holdersCount == null ? "—" : holdersCount.toLocaleString("ru-RU")} />
            <StatChip icon={Flame} label={tr("statVolume24h")} value={`$${t.vol}`} />
            <StatChip icon={Clock} label={tr("statTransactions1h")} value={(t.txCount1h || 0).toLocaleString("ru-RU")} />
          </div>
      ...
      {tab === "tx" && (
        <div className="flex flex-col gap-1.5">
          {!t.poolAddress || (!tradesLoading && trades && trades.length === 0) || (!tradesLoading && !trades) ? (
            <div className="rounded-[22px] p-4 flex items-center justify-center text-center" style={{ background: T.surface, border: `1px dashed ${T.line}`, minHeight: 80 }}>
              <span style={{ fontFamily: bodyFont, color: T.muted, fontSize: 12.5 }}>{tr("txUnavailable")}</span>
            </div>
      ...
      {error ? (
        <div className="fx-modal-card flex flex-col items-center text-center gap-4" style={{ width: "100%", maxWidth: 340, background: T.surface, border: `1px solid ${T.lineHi}`, borderRadius: 24, padding: 24 }}>
          ...
          <div>
            ...
            <div style={{ fontFamily: bodyFont, color: T.muted, fontSize: 12, marginTop: 6, lineHeight: 1.5 }}>{error}</div>
          </div>
...
function CreateView({ showToast, unlocked, accountCreated, connected, onOpenCreateProfile, onOpenConnectModal, onLaunch }) {
  ...
  return (
    <div className="fx-view flex flex-col gap-7 pb-36" style={{ position: "relative" }}>
      ...
      <div>
        <span style={{ fontFamily: bodyFont, color: T.muted, fontSize: 12 }}>{t("categoryLabel")}</span>
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <div className="flex items-center gap-2 rounded-[20px] px-3.5 py-3" style={{ background: T.surface, border: `1px solid ${touched && !(parseFloat(form.buyAmount.replace(",", ".")) > 0) ? T.down : T.line}` }}>
          <input
...
function WalletCard({ connected, walletAddress, tonBalance = 0, tonPriceUsd = 0, onConnect, onDisconnect, onCopy, onExplore }) {
  ...
  return (
    <GlassCard style={{ padding: 20 }} className="fx-view">
      <SectionTitle action={<span style={{ fontFamily: monoFont, fontSize: 10, color: T.turquoise, display: "flex", alignItems: "center", gap: 4 }}><span style={{ width: 6, height: 6, borderRadius: "50%", background: T.turquoise, animation: "ringPulse 1.8s ease-out infinite" }} /> {t("connected")}</span>}>
        {t("walletProvider")}
      </SectionTitle>
      <div className="flex items-end gap-2">
  ...
  return (
    <GlassCard style={{ padding: "12px 14px" }} className="flex items-center gap-3">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div style={{ fontFamily: bodyFont, color: T.muted, fontSize: 10.5, marginTop: 2 }}>{tr("liqShort")} ${t.liq} · {t.holders.toLocaleString("ru-RU")} {tr("holdersShort")} · {tr("volShort")} {t.vol}</div>
      </div>
    ...
    <div className="fx-view" style={{ position: "relative" }}>
      <div className="flex flex-col gap-0 pb-4">
        ...
        <div className="mt-5">
          ...
          <div className="grid grid-cols-2 gap-2">
            <StatBlock label={t("statPortfolioValue")} value={0} color={T.turquoise} />
            ...
            <StatBlock label={t("statWinRate")} value={0} suffix="%" />
        ...
        <div className="mt-5">
          <SectionTitle action={<button onClick={goCreateToken} className="fx-tap flex items-center gap-1" style={{ fontFamily: bodyFont, fontSize: 11.5, color: unlocked ? T.electric : T.muted }}>{unlocked ? <PlusCircle size={13} /> : <Lock size={12} />} {t("myTokensCreate")}</button>}>{t("myTokensTitle")}</SectionTitle>
...
const FEE_PERCENT = 0.01; // 1% комиссии
  ...
  // Real, live TON meme-pool feed — polled at TOKEN_REFRESH_MS. Lives at
  // the root (not inside HomeView) so the currently-open token detail can
  // also stay synced to fresh data below, instead of freezing at whatever
  // price it had the moment it was tapped.
  const [tokens, setTokens] = useState([]);
  const [tokensLoading, setTokensLoading] = useState(true);
  // feedError only ever gets set to a real message when the feed is
  // *actually empty* (no tokens fetched yet at all) — once tokens has
  // loaded once, later transient failures just keep showing the last
  // good list silently, exactly like before.
  ...
  useEffect(() => {
    ...
    async function poll() {
      ...
      if (live && live.length) {
        ...
        setFeedError(lastFeedErrorMsg || "Не удалось получить список токенов");
      }
  ...
  // "My Tokens" (create-flow results) — purely local, in-memory + localStorage.
  // No token here was ever deployed anywhere; each entry is produced entirely
  // client-side by TokenLaunchOverlay's simulated sequence.
  ...
  function handleTokenCreated(result) {
    ...
    const entry = {
      ...
      vol: "$0",
      change: 0,
      seed: hashSeed(`local-${result.createdAt}`),
    ...
    setMyTokens((prev) => {
      ...
      try {
        if (typeof window !== "undefined") window.localStorage.setItem("mintly_my_tokens", JSON.stringify(next));
      } catch (e) { /* localStorage unavailable */ }
      return next;
    });
  }
      ...
      <div style={{ position: "relative", zIndex: 1, height: "100%", display: "flex", flexDirection: "column" }}>
        <div
          ...
          style={{
            ...
            background: hexA(T.bg, 0.08), backdropFilter: "blur(24px)", WebkitBackdropFilter: "blur(24px)",
          }}
        >
          <button onClick={() => goTab("home")} className="fx-tap flex items-center gap-1.5">
          <span style={{ fontFamily: logoFont, color: appSettings.theme === "White" ? T.ice : T.turquoise, fontSize: 26, fontWeight: 600, fontStyle: "italic", letterSpacing: "0em" }}>Mintly</span>
          </button>
          <button onClick={handleHeaderWalletClick} className="fx-tap fx-chip flex items-center gap-1.5 rounded-full px-3 py-1.5" style={{ background: T.surface, border: `1px solid ${connected ? glow(0.35) : T.line}` }}>
        ...
        {/* only this area scrolls — header floats above it (position: absolute)
            so content actually passes underneath and shows through the blur;
            paddingTop below reserves the header's own height so content
            doesn't start out hidden behind it. Bottom nav stays pinned. */}
        <div className="no-scrollbar px-4" style={{ flex: 1, overflowY: "auto", minHeight: 0, paddingTop: 64 }} key={view}>
          {view === "home" && <HomeView onGoTab={goTab} />}
          {view === "mempad" && <MempadView tokens={tokens} myTokens={myTokens} loading={tokensLoading} error={feedError} onRetry={() => { setTokensLoading(true); setFeedRetryTick(n => n + 1); }} onOpen={openToken} onLaunch={() => goTab("create")} />}
        ...
        <div className="flex items-center justify-around px-2 py-4" style={{ borderTop: `1px solid ${T.line}`, background: "transparent", flexShrink: 0, paddingBottom: insetBottom + 16 }}>
          {[
