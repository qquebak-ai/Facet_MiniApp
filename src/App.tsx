...
function MempadView({ tokens, loading, error, onRetry, onOpen, onLaunch }) {
  ...
  // demo data. The three tabs below just re-slice that same real feed:
  //  - dex:  actually listed & liquid on a DEX right now, ranked by real
  //          pooled liquidity (highest first) — the "safest"/most-traded view.
  //  - new:  ranked by the pool's real on-chain creation timestamp,
  //          newest first.
  ...
  const list = useMemo(() => {
    ...
    switch (filter) {
      case "hot":
        ...
        break;
      case "new":
        arr = [...arr].sort((a, b) => (b.createdAtMs || 0) - (a.createdAtMs || 0));
        break;
      default: // dex
    ...
    <div style={{ background: T.bg, height, minHeight: height, width: "100%", maxWidth: 480, margin: "0 auto", fontFamily: bodyFont, position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ position: "relative", zIndex: 1, height: "100%", display: "flex", flexDirection: "column" }}>
        ...
        <div className="no-scrollbar px-4" style={{ flex: 1, overflowY: "auto", minHeight: 0, paddingTop: 64 }} key={view}>
          ...
          {view === "mempad" && <MempadView tokens={tokens} loading={tokensLoading} error={feedError} onRetry={() => { setTokensLoading(true); setFeedRetryTick(n => n + 1); }} onOpen={openToken} onLaunch={() => goTab("create")} />}
