...
// SimpleCandleChart — lightweight, dependency-free SVG line+area chart.
// Used only for bundled demo tokens without a real on-chain pool (real
// pools get the embedded GeckoTerminal widget instead, see below).
function SimpleCandleChart({ candles, height = 230 }) {
  if (!candles?.length) return null;
  const width = 600;
  ...
  const stepX = candles.length > 1 ? width / (candles.length - 1) : width;
  const points = candles.map((c, i) => {
    const x = i * stepX;
    const y = padY + (1 - (c.close - min) / range) * (height - padY * 2);
    return [x, y];
  });
...
// LivePoolChart — real, live price chart for tokens backed by an actual
// TON on-chain pool. Rather than maintaining our own candlestick renderer
// (fragile against odd API data), this embeds GeckoTerminal's own chart
// widget for the exact same pool — it handles zoom/pan/timeframes and
// stays visually correct on its own. `light_chart` follows the app's
// Dark/White theme so it doesn't clash with the surrounding UI.
function LivePoolChart({ poolAddress, height = 400, themeKey }) {
  const src = `https://www.geckoterminal.com/${GT_NETWORK}/pools/${poolAddress}?embed=1&info=0&swaps=0&light_chart=${themeKey === "White" ? 1 : 0}`;
  return (
    <iframe
      title="price-chart"
      src={src}
...
function TokenDetail({ t, onBack, showToast, onBuy, onSell, unlocked = true, themeKey }) {
  ...
  const [chartData, setChartData] = useState(null); // { candles, volume } — demo tokens only
  const [chartLoading, setChartLoading] = useState(true);
  const up = t.change >= 0;
  const isLive = !!t.poolAddress;

  // Tokens backed by a real on-chain pool get the embedded GeckoTerminal
  // widget (see LivePoolChart) instead — it fetches and renders its own
  // data, so we only need a synthetic random-walk chart here for bundled
  // demo tokens without a real pool.
  useEffect(() => {
    if (isLive) { setChartLoading(false); return; }
    setChartLoading(true);
    const result = genSyntheticCandles(t.price, t.seed, tf, CHART_TOTAL);
    setChartData(result);
    setChartLoading(false);
  }, [tf, t.id, t.price, t.seed, isLive]);

  // Simulated live tick for the synthetic fallback only — wiggles the
  // last candle locally so the demo chart doesn't look frozen.
  useEffect(() => {
    ...
    <div className="fx-view flex flex-col gap-4 pb-4" style={{ position: "relative" }}>
      ...
      {isLive ? (
        <div className="rounded-2xl overflow-hidden" style={{ background: T.surface, border: `1px solid ${T.line}` }}>
          <div className="flex items-center gap-1.5 flex-wrap px-2.5 pt-2.5 pb-1.5">
            <span style={{ fontSize: 13 }}>{t.emoji}</span>
            <span style={{ fontFamily: displayFont, color: T.ice, fontSize: 12.5, fontWeight: 700 }}>{t.name}</span>
            <span style={{ width: 5, height: 5, borderRadius: "50%", background: T.up, boxShadow: `0 0 5px ${T.up}` }} />
            <span style={{ fontFamily: monoFont, color: T.muted, fontSize: 10 }}>GeckoTerminal · live</span>
          </div>
          <LivePoolChart poolAddress={t.poolAddress} themeKey={themeKey} />
        </div>
      ) : (
        <>
          ...
          <div className="rounded-2xl overflow-hidden" style={{ background: T.surface, border: `1px solid ${T.line}`, padding: "10px 10px 4px" }} key={tf}>
            ...
            {chartLoading || !chartData ? (
              ...
              <SimpleCandleChart candles={chartData.candles} />
            )}
          </div>
        </>
      )}
