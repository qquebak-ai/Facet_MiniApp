        ...
        : items.map((it, i) => (
            <span
              ...
              style={{
                ...
                background: T.down,
                boxShadow: `0 0 6px 1.5px ${hexA(T.down, 0.55)}`,
                animation: `emberFall ${it.dur}s ease-in ${it.delay}s infinite`,
              }}
            />
          ))}
    </div>
    ...
    <div className="fx-view flex flex-col gap-4 pb-4" style={{ position: "relative" }}>
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex flex-col items-end gap-1" style={{ fontFamily: bodyFont, fontSize: 12, paddingTop: 2 }}>
          <div className="flex items-center gap-2"><span style={{ color: T.muted }}>{tr("statHolders")}</span><span style={{ fontFamily: monoFont, color: T.ice }}>{token.holders.toLocaleString("ru-RU")}</span></div>
          <div className="flex items-center gap-2"><span style={{ color: T.muted }}>{tr("statTransactions")}</span><span style={{ fontFamily: monoFont, color: T.ice }}>{(token.txCount24h || 0).toLocaleString("ru-RU")}</span></div>
      ...
      {tab === "info" && (
        <div className="flex flex-col gap-3">
          <div className="grid grid-cols-2 gap-2">
            ...
            <StatChip icon={User} label={tr("statHolders")} value={token.holders.toLocaleString("ru-RU")} />
