...
const TF_MS = { M1: 60e3, M5: 5 * 60e3, M15: 15 * 60e3, M30: 30 * 60e3, H1: 3600e3, H4: 4 * 3600e3, D1: 86400e3, W1: 7 * 86400e3, MN1: 30 * 86400e3 };

function fmtAxisTime(ms, timeframe) {
  const d = new Date(ms);
...
function genCandles(basePrice, seed, timeframe, n = 64) {
  ...
  const stepMs = TF_MS[timeframe] || TF_MS.H1;
  ...
  for (let i = 0; i < n; i++) {
    ...
    const volume = (body / vol) * 0.6 + Math.random() * 0.55 + 0.15;
    const time = anchor - (n - 1 - i) * stepMs;
    out.push({ i, open, high, low, close, volume, time });
    price = close;
  }
  return out;
}

const CandlestickChart = React.memo(function CandlestickChart({ data, width = 340, height = 210, tf = "H1", crosshairIndex = null }) {
  const padL = 4, padR = 46, padT = 10;
  const axisH = 14;      // bottom time-label strip
  const gapVol = 6;      // gap between price pane and volume pane
  const volH = Math.max(24, Math.round(height * 0.16));
  const plotW = width - padL - padR;
  const mainPlotH = height - padT - gapVol - volH - axisH;
  const volY0 = padT + mainPlotH + gapVol;
  ...
  const maxVol = Math.max(...data.map(d => d.volume), 0.0001);

  function y(v) { return padT + (1 - (v - min) / range) * mainPlotH; }
  function volY(v) { return volY1 - (v / maxVol) * volH; }
  ...
  // Time-axis ticks: a handful of evenly spaced labels under the visible range.
  const timeTickCount = width < 300 ? 3 : 4;
  const timeTicks = useMemo(() => {
    const out = [];
    for (let k = 0; k < timeTickCount; k++) {
      const idx = Math.round((k / (timeTickCount - 1)) * (data.length - 1));
      out.push(data[idx]);
    }
    return out;
  }, [data, timeTickCount]);

  // Track which candle indices have already played their "grow" entrance
  // animation. Without this, every 1s live-price tick re-renders the whole
  // chart and CSS restarts the animation (with its staggered delay) on
  // EVERY candle again — that's the stutter/flicker people perceive as
  // lag. Now only candles that are genuinely new to view (new timeframe,
  // newly panned/zoomed into view) animate; a tick update just repaints
  // the last candle's geometry instantly, no animation replay.
  const seenRef = useRef(new Set());
  useEffect(() => {
    data.forEach(c => seenRef.current.add(c.i));
  });

  // The candle bodies/wicks + volume bars are the expensive part to paint
  // (up to ~140 SVG nodes). Memoize that geometry on data/dimensions alone
  // so that crosshair movement — which can fire on every pointermove — only
  // repaints the thin overlay layer below, not the whole chart.
  const candleLayer = useMemo(() => (
    <>
      {data.map((c, i) => {
        ...
        const isNew = !seenRef.current.has(c.i);
        const vH = Math.max(1, volY1 - volY(c.volume));
        return (
          <g key={c.i} style={isNew ? { transformOrigin: `${x}px ${volY1}px`, animation: `candleGrow 420ms cubic-bezier(0.16,1,0.3,1) both`, animationDelay: `${i * 8}ms` } : undefined}>
            ...
            <rect x={x - bodyW / 2} y={volY(c.volume)} width={bodyW} height={vH} rx="1" fill={color} opacity="0.32" />
          </g>
        );
      })}
    </>
  ...
  const cx = crosshairIndex != null ? data[crosshairIndex] : null;
  ...
  return (
    <svg width="100%" height={height} viewBox={`0 0 ${width} ${height}`} style={{ display: "block", overflow: "visible" }}>
      ...
      <line x1={padL} x2={width - padR} y1={volY1} y2={volY1} stroke={T.line} strokeWidth="1" />
      ...
      {/* bottom time axis */}
      {timeTicks.map((c, idx) => {
        const x = xAt(c.i - data[0].i);
        const anchor = idx === 0 ? "start" : idx === timeTicks.length - 1 ? "end" : "middle";
        return (
          <text key={idx} x={Math.min(Math.max(x, padL + 10), width - padR - 10)} y={height - 3} fontFamily={monoFont} fontSize="8.5" fill={T.muted} textAnchor={anchor}>
            {fmtAxisTime(c.time, tf)}
          </text>
        );
      })}
      ...
      <g>
        ...
        <text x={width - padR + 3 + tagW / 2} y={yLast + 3} fontFamily={monoFont} fontSize="8.5" fontWeight="700" fill="#08080A" textAnchor="middle">
          {fmtPrice(last.close)}
        </text>
      </g>

      {/* crosshair overlay — appears on press/hold, tracks the touched candle */}
      {cx && (() => {
        const x = xAt(crosshairIndex);
        const yPrice = y(cx.close);
        const priceLabel = fmtPrice(cx.close);
        const timeLabel = fmtAxisTime(cx.time, tf);
        const estCharW = 5.1;
        const priceTagW = priceLabel.length * estCharW + 10;
        const timeTagW = timeLabel.length * estCharW + 10;
        const timeTagX = Math.min(Math.max(x - timeTagW / 2, padL), width - padR - timeTagW);
        return (
          <g pointerEvents="none">
            <line x1={x} x2={x} y1={padT} y2={volY1} stroke={T.ice} strokeWidth="1" strokeDasharray="2 3" opacity="0.55" />
            <line x1={padL} x2={width - padR} y1={yPrice} y2={yPrice} stroke={T.ice} strokeWidth="1" strokeDasharray="2 3" opacity="0.55" />
            <circle cx={x} cy={yPrice} r="2.5" fill={T.ice} />

            <rect x={width - padR + 3} y={yPrice - 8} width={Math.max(tagW, priceTagW)} height={16} rx="3" fill={T.ice} />
            <text x={width - padR + 3 + Math.max(tagW, priceTagW) / 2} y={yPrice + 3} fontFamily={monoFont} fontSize="8.5" fontWeight="700" fill={T.bg} textAnchor="middle">
              {priceLabel}
            </text>

            <rect x={timeTagX} y={height - axisH - 2} width={timeTagW} height={13} rx="3" fill={T.ice} />
            <text x={timeTagX + timeTagW / 2} y={height - 2} fontFamily={monoFont} fontSize="8.5" fontWeight="700" fill={T.bg} textAnchor="middle">
              {timeLabel}
            </text>
          </g>
        );
...
function TokenDetail({ t, onBack, showToast, onBuy, onSell, unlocked = true }) {
  ...
  const up = t.change >= 0;
  // crosshairIndex is relative to the currently *visible* candle slice (0..count-1),
  // matching what CandlestickChart is handed. null = crosshair hidden.
  ...
  useEffect(() => {
    ...
    // Long-press-to-crosshair: a finger held roughly in place enters
    // crosshair/inspect mode (panning suspends, the touched candle is
    // highlighted); a finger that moves past the deadzone before the
    // hold timer fires is treated as a normal drag-to-pan instead.
    const HOLD_MS = 140, DEADZONE = 6;
    let holdTimer = null;
    let holdStart = null;
    let crosshairActive = false;
    let pendingCrosshair;
    let crosshairRafId = null;

    const clamp = (v, lo, hi) => Math.max(lo, Math.min(hi, v));
    const dist = (a, b) => Math.hypot(a.x - b.x, a.y - b.y);

    // Pointer/wheel events can fire 60-120+ times per second while dragging
    // or pinching on mobile. Calling setView() straight from the handler
    // (as before) forced a full React re-render — and a re-slice of up to
    // 140 candles — on every single one of those events, which is what
    // made panning/zooming feel laggy. Now we just record the latest
    // requested view and commit a single setView per animation frame,
    // capping updates to the screen's actual refresh rate.
    ...
    function applyCrosshair(idx) {
      pendingCrosshair = idx;
      if (crosshairRafId == null) {
        crosshairRafId = requestAnimationFrame(() => {
          crosshairRafId = null;
          setCrosshairIndex(pendingCrosshair);
        });
      }
    }
    ...
    function indexFromClientX(clientX) {
      ...
      return clamp(Math.round((clientX - rect.left) / pxPerCandle), 0, viewRef.current.count - 1);
    }
    ...
    function endCrosshair() {
      clearHold();
      if (crosshairActive) { crosshairActive = false; applyCrosshair(null); }
    }
    ...
    function onPointerDown(e) {
      ...
      if (pointers.size === 1) {
        ...
        holdTimer = setTimeout(() => {
          ...
          applyCrosshair(indexFromClientX(e.clientX));
        }, HOLD_MS);
    ...
    function onPointerMove(e) {
      ...
      } else if (pointers.size === 1 && crosshairActive) {
        applyCrosshair(indexFromClientX(e.clientX));
      } else if (pointers.size === 1 && dragStartX !== null) {
        if (holdTimer != null && holdStart && dist(holdStart, { x: e.clientX, y: e.clientY }) > DEADZONE) {
          clearHold();
        }
        const rect = el.getBoundingClientRect();
        const pxPerCandle = rect.width / viewRef.current.count;
  ...
  function resetZoom() { setView({ start: CHART_TOTAL - CHART_DEFAULT_VISIBLE, count: CHART_DEFAULT_VISIBLE }); setCrosshairIndex(null); }
  ...
  return (
    <div className="fx-view flex flex-col gap-4 pb-4" style={{ position: "relative" }}>
      ...
      <div className="rounded-2xl overflow-hidden" style={{ background: T.surface, border: `1px solid ${T.line}`, padding: "10px 10px 4px" }} key={tf}>
        ...
        {(() => {
          const hovered = crosshairIndex != null ? visibleCandles[crosshairIndex] : null;
          const first = visibleCandles[0], last = visibleCandles[visibleCandles.length - 1];
          const rowOpen = hovered ? hovered.open : first.open;
          const rowHigh = hovered ? hovered.high : Math.max(...visibleCandles.map(c => c.high));
          const rowLow = hovered ? hovered.low : Math.min(...visibleCandles.map(c => c.low));
          const rowClose = hovered ? hovered.close : last.close;
          const delta = rowClose - rowOpen;
          const deltaPct = rowOpen ? (delta / rowOpen) * 100 : 0;
          ...
          return (
            <div className="flex items-center gap-2.5 flex-wrap" style={{ fontFamily: monoFont, fontSize: 10, color: T.muted, marginTop: 5, marginBottom: 6 }}>
              <span>ОТКР <span style={{ color: T.ice }}>{fmtPrice(rowOpen)}</span></span>
              <span>МАКС <span style={{ color: T.ice }}>{fmtPrice(rowHigh)}</span></span>
              <span>МИН <span style={{ color: T.ice }}>{fmtPrice(rowLow)}</span></span>
              <span>ЗАКР <span style={{ color: T.ice }}>{fmtPrice(rowClose)}</span></span>
        ...
        <div ref={chartWrapRef} style={{ touchAction: "none", userSelect: "none", WebkitUserSelect: "none", WebkitTouchCallout: "none" }}>
          <CandlestickChart data={visibleCandles} tf={tf} crosshairIndex={crosshairIndex} />
        </div>
      </div>
