...
const LWChart = React.memo(function LWChart({ candles, volume, height = 230, themeKey, resetKey }) {
  ...
  // Push new data whenever it changes. Live-tick refreshes (same
  // token/timeframe, just newer candles) must NOT reset the visible
  // range — that's what made the chart feel "locked": every periodic
  // refetch called fitContent() and yanked the view back regardless of
  // where the user had scrolled/zoomed to. Now the view only auto-fits
  // once, on a genuine new load (resetKey changes = new token or new
  // timeframe); every later data push leaves pan/zoom exactly where the
  // user left it, so the chart scrolls/zooms completely freely.
  useEffect(() => {
    ...
    <div className="fx-view flex flex-col gap-4 pb-4" style={{ position: "relative" }}>
      ...
      <div className="rounded-2xl overflow-hidden" style={{ background: T.surface, border: `1px solid ${T.line}`, padding: "10px 10px 4px" }} key={tf}>
        ...
        {chartLoading || !chartData ? (
          ...
          <LWChart candles={chartData.candles} volume={chartData.volume} themeKey={themeKey} resetKey={`${t.id}-${tf}`} />
        )}
      </div>
