...
// GeckoTerminal's placeholder for tokens that have no real logo uploaded —
// filtered out so the UI falls back to the emoji instead of showing GT's
// generic "missing" icon as if it were the token's real avatar.
function isRealTokenImage(url) {
  return typeof url === "string" && url.length > 0 && !/missing/i.test(url);
}

async function fetchTonMemePools(limit = 18) {
  try {
    // include=base_token pulls each pool's underlying token metadata (name,
    // symbol, and — importantly — its real logo image_url) into the
    // response's top-level "included" array, so the app can show the
    // actual memtoken avatar instead of a generic emoji.
    ...
    const tokensById = new Map((json?.included || []).map(inc => [inc.id, inc.attributes || {}]));
    return rows.map((row) => {
      const a = row.attributes || {};
      const baseTokenId = row.relationships?.base_token?.data?.id;
      const baseToken = baseTokenId ? tokensById.get(baseTokenId) : null;
      ...
      return {
        ...
        image: isRealTokenImage(baseToken?.image_url) ? baseToken.image_url : null,
        price,
        change,
        mcapNum,
...
function genSyntheticCandles(basePrice, seed, timeframe, n = 140) {
  ...
  const stepSec = { M1: 60, M5: 300, M15: 900, M30: 1800, H1: 3600, H4: 14400, D1: 86400, W1: 604800, MN1: 2592000 }[timeframe] || 3600;
...
const CHART_AXIS_W = 54;     // reserved strip on the right for the price ribbon
const CHART_MIN_SCALE = 0.15;
const CHART_MAX_SCALE = 6;

function TerminalChart({ candles, height = 340, themeKey, onHover }) {
  ...
  // Manual vertical adjustments layered on top of the auto-fit min/max of
  // whatever candles are currently visible — this is what lets the price
  // scale be panned/zoomed freely instead of always snapping back to fit
  // the data.
  const yScaleRef = useRef(1);        // multiplies the auto-fit price range (the ribbon's zoom)
  const yOffsetRef = useRef(0);       // shifts the center price (vertical pan)
  const lastRangeRef = useRef(null);  // fallback {min,max} for when panned into empty space
  const dragRef = useRef(null);       // { lastX, lastY, lastT, vx, moved, startX, startY }
  const pinchRef = useRef(null);      // { startDist, startCount, anchorIdx }
  const axisDragRef = useRef(null);   // { startY, startScale } — dragging the right-hand price ribbon
  ...
  // Only the zoom level (how many candles fit on screen) is bounded — the
  // scroll position itself is completely free, in both time and price, so
  // the view can pan past the first/last candle into open space just like
  // a normal exchange chart.
  function clampView() {
    const v = viewRef.current;
    const maxCount = Math.max(n, CHART_MIN_VISIBLE) * 3;
    v.count = Math.max(CHART_MIN_VISIBLE, Math.min(maxCount, v.count || CHART_DEFAULT_VISIBLE));
    yScaleRef.current = Math.max(CHART_MIN_SCALE, Math.min(CHART_MAX_SCALE, yScaleRef.current));
  }
  ...
  function computeLayout() {
    ...
    const visible = candles.slice(startI, endI).filter(Boolean);
    const plotW = Math.max(1, widthPx - CHART_AXIS_W);
    const slot = plotW / count;
    const bodyW = Math.max(2, Math.min(14, slot * 0.7));

    let autoMin, autoMax;
    if (visible.length) {
      autoMin = Math.min(...visible.map(c => c.low));
      autoMax = Math.max(...visible.map(c => c.high));
      lastRangeRef.current = { min: autoMin, max: autoMax };
    } else if (lastRangeRef.current) {
      // Panned into empty space (before the first or after the last
      // candle) — keep the last known price range instead of collapsing.
      autoMin = lastRangeRef.current.min;
      autoMax = lastRangeRef.current.max;
    } else {
      return null;
    }
    const autoRange = (autoMax - autoMin) || (autoMax * 0.02) || 1;
    const range = autoRange * yScaleRef.current;
    const center = (autoMax + autoMin) / 2 + yOffsetRef.current;
    ...
    return { startI, endI, min, max, range, slot, bodyW, plotW, yFor, xFor };
  }

  // Sizing the canvas (setting .width/.height) forces the browser to
  // reallocate its entire backing buffer — fine once, but disastrous if
  // done on every single draw() call during a drag (which can fire 60+
  // times/sec). This only touches the canvas element when the actual
  // pixel size changed (resize, or height/DPI change), never during a
  // plain redraw. DPR is also capped at 2 — 3x on a phone quadruples the
  // pixel count for a chart that doesn't need that much sharpness.
  ...
  function ensureCanvasSize() {
    ...
    if (canvas && (s.w !== w || s.h !== h || s.dpr !== dpr)) {
      canvas.width = w;
      canvas.height = h;
  ...
  function draw() {
    ...
    const { startI, endI, min, range, yFor, xFor, bodyW, plotW } = layout;

    // Candles — high-contrast bodies + wicks, sized to fill the available
    // width so they stay readable at any zoom level. No grid lines across
    // the plot area — just the candles themselves.
    ...
    // Price ribbon — a reserved strip on the right, like a normal exchange
    // chart. Dragging it up/down zooms the price scale (narrower / wider);
    // no lines are drawn across the candles, just labels in their own lane.
    ctx.fillStyle = hexA(T.surface, 0.92);
    ctx.fillRect(plotW, 0, CHART_AXIS_W, height);
    ctx.strokeStyle = hexA(T.line, 0.6);
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.moveTo(plotW, 0);
    ctx.lineTo(plotW, height);
    ctx.stroke();
    ...
    for (let i = 0; i <= levels; i++) {
      const price = min + (range * i) / levels;
      const y = yFor(price);
      if (y < 8 || y > height - 4) continue;
      ctx.strokeStyle = hexA(T.line, 0.6);
      ctx.beginPath();
      ctx.moveTo(plotW, y);
      ctx.lineTo(plotW + 5, y);
      ctx.stroke();
      ctx.fillStyle = T.muted;
      ctx.fillText(fmtPrice(price), plotW + 8, y + 3);
    }

    // Last price — a dashed marker across the plot with a filled label on
    // the ribbon, so the current price stands out the way it does on any
    // normal candlestick chart.
    const lastCandle = candles[candles.length - 1];
    if (lastCandle) {
      const ly = yFor(lastCandle.close);
      if (ly > 0 && ly < height) {
        const lastColor = lastCandle.close >= lastCandle.open ? T.up : T.down;
        ctx.strokeStyle = hexA(lastColor, 0.7);
        ctx.setLineDash([4, 3]);
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.moveTo(0, ly);
        ...
        ctx.fillRect(plotW, ly - 8, CHART_AXIS_W, 16);
        ctx.fillStyle = "#fff";
        ctx.fillText(fmtPrice(lastCandle.close), plotW + 8, ly + 3);
      }
    }
  ...
  // touchmove/mousemove/wheel can all fire faster than the screen actually
  // repaints — calling draw() straight from the handler wastes work
  // redrawing frames nobody ever sees. scheduleDraw() coalesces any number
  // of calls within one frame down to a single paint, which is what keeps
  // dragging smooth on phones instead of stacking up redraw work.
  const rafPendingRef = useRef(false);
  function scheduleDraw() {
    if (rafPendingRef.current) return;
    rafPendingRef.current = true;
    requestAnimationFrame(() => { rafPendingRef.current = false; draw(); });
  }
  ...
  function inAxisZone(clientX) {
    return xFromEvent(clientX) > widthPx - CHART_AXIS_W;
  }
  function cancelInertia() {
    if (inertiaRaf.current) cancelAnimationFrame(inertiaRaf.current);
    inertiaRaf.current = null;
  }
  function panByPixels(dxScreen, dyScreen) {
    const layout = computeLayout();
    if (!layout) return;
    viewRef.current.start -= dxScreen / layout.slot;
    if (dyScreen) {
      // Vertical drag pans the price window freely too — same unrestricted
      // movement as panning through time.
      yOffsetRef.current += (dyScreen / (height - height * 0.18)) * layout.range;
    }
    clampView();
    scheduleDraw();
  }
  ...
  function onTouchStart(e) {
    ...
    if (inAxisZone(x)) {
      axisDragRef.current = { startY: y, startScale: yScaleRef.current };
      return;
    }
    dragRef.current = { lastX: x, lastY: y, lastT: performance.now(), vx: 0, moved: false, startX: x, startY: y };
  }
  function onTouchMove(e) {
    if (e.touches.length === 2 && pinchRef.current) {
      ...
      newCount = Math.max(CHART_MIN_VISIBLE, Math.min(Math.max(n, CHART_MIN_VISIBLE) * 3, newCount));
      const mx = midX(e.touches);
      const plotW = Math.max(1, widthPx - CHART_AXIS_W);
      const newSlot = plotW / newCount;
      viewRef.current = { start: pinchRef.current.anchorIdx - mx / newSlot, count: newCount };
      clampView();
      scheduleDraw();
      return;
    }
    if (axisDragRef.current && e.touches.length === 1) {
      e.preventDefault();
      const dy = e.touches[0].clientY - axisDragRef.current.startY;
      // Dragging the ribbon down narrows the visible price range (zoom
      // in); dragging up widens it (zoom out) — same as normal charts.
      yScaleRef.current = axisDragRef.current.startScale * Math.exp(dy / 160);
      clampView();
      scheduleDraw();
      return;
    }
  ...
  function onMouseDown(e) {
    cancelInertia();
    if (inAxisZone(e.clientX)) {
      axisDragRef.current = { startY: e.clientY, startScale: yScaleRef.current };
      return;
    }
    dragRef.current = { lastX: e.clientX, lastY: e.clientY, lastT: performance.now(), vx: 0, moved: false, startX: e.clientX, startY: e.clientY };
  }
  function onMouseMove(e) {
    if (axisDragRef.current) {
      if (e.buttons !== 1) { axisDragRef.current = null; return; }
  ...
  function onWheel(e) {
    e.preventDefault();
    if (inAxisZone(e.clientX)) {
      // Wheel over the ribbon zooms the price scale instead of time.
      yScaleRef.current *= e.deltaY > 0 ? 1.08 : 0.92;
      clampView();
      scheduleDraw();
      return;
    }
    ...
    let newCount = Math.max(CHART_MIN_VISIBLE, Math.min(Math.max(n, CHART_MIN_VISIBLE) * 3, viewRef.current.count * factor));
    const newSlot = layout.plotW / newCount;
...
function TokenAvatar({ children, size = 52, tone = "neutral", image }) {
  // Real logo when one was fetched (e.g. from GeckoTerminal's token data);
  // falls back to the emoji if there's no image, or if it fails to load —
  // tracked per-image so switching tokens (new `image` prop) always gets a
  // fresh chance to load rather than staying stuck on a prior failure.
  ...
  return (
    <div className="fx-avatar" style={{ width: size, height: size, position: "relative", flexShrink: 0 }}>
      ...
      <div style={{
        ...
        {showImage
          ? <img src={image} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", borderRadius: "50%" }} onError={() => setFailed(true)} />
          : children}
      </div>
    </div>
  );
    ...
    <button onClick={() => onOpen(t)} className="fx-card w-full text-left rounded-2xl" style={{ background: T.surface, border: `1px solid ${up ? "rgba(49,208,123,0.28)" : "rgba(255,77,77,0.24)"}`, padding: "9px 12px", animationDelay: `${index * 55}ms`, position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ position: "relative", overflow: "hidden", borderRadius: 10 }}>
        ...
        <div className="flex items-center gap-2.5" style={{ position: "relative", zIndex: 1 }}>
          <TokenAvatar size={42} tone={up ? "up" : "down"} image={t.image}>{t.emoji}</TokenAvatar>
...
function HomeView({ onOpen, onSearch }) {
  ...
  // Starts empty — the skeleton cards below cover the loading gap, so
  // there's never a moment where invented placeholder tokens are shown.
  // Only the real, live-fetched pools ever populate this list.
  ...
  const list = useMemo(() => {
    ...
    switch (filter) {
      ...
      default: break; // gems / whale / volume / followed / community: показываем весь пул
    }
    if (query.trim()) {
...
function TokenDetail({ t, onBack, showToast, onBuy, onSell, unlocked = true, themeKey }) {
  ...
  return (
    <div className="fx-view flex flex-col gap-4 pb-4" style={{ position: "relative" }}>
      ...
      <div className="flex items-center justify-between">
        <button onClick={onBack} className="fx-tap flex items-center gap-1" style={{ color: T.muted, fontFamily: bodyFont, fontSize: 13 }}><ChevronLeft size={16} /> {tr("back")}</button>
      ...
      <div className="flex items-center gap-3">
        <TokenAvatar size={56} tone={up ? "up" : "down"} image={t.image}>{t.emoji}</TokenAvatar>
        <div>
          <div className="flex items-center gap-1.5">
      ...
      <div className="rounded-2xl overflow-hidden" style={{ background: T.surface, border: `1px solid ${T.line}`, padding: "8px 8px 2px" }}>
        <div className="flex items-center gap-1.5 flex-wrap" style={{ padding: "0 2px" }}>
          <span style={{ fontSize: 13 }}>{t.emoji}</span>
          <span style={{ fontFamily: displayFont, color: T.ice, fontSize: 12.5, fontWeight: 700 }}>{t.name}</span>
          <span style={{ fontFamily: monoFont, color: T.muted, fontSize: 10 }}>· {tf}</span>
          {chartData?.isLive && <span style={{ width: 5, height: 5, borderRadius: "50%", background: T.up, boxShadow: `0 0 5px ${T.up}`, marginLeft: "auto" }} />}
        </div>
        {chartData && (() => {
          const { candles } = chartData;
          const ref = hovered || candles[candles.length - 1];
          const first = candles[0];
          const delta = ref.close - first.open;
          const deltaPct = first.open ? (delta / first.open) * 100 : 0;
          const periodUp = delta >= 0;
          return (
            <div className="flex items-center gap-2.5 flex-wrap" style={{ fontFamily: monoFont, fontSize: 10, color: T.muted, marginTop: 3, marginBottom: 4, padding: "0 2px" }}>
              <span>{tr("ohlcOpen")} <span style={{ color: T.ice }}>{fmtPrice(ref.open)}</span></span>
              <span>{tr("ohlcHigh")} <span style={{ color: T.ice }}>{fmtPrice(ref.high)}</span></span>
              <span>{tr("ohlcLow")} <span style={{ color: T.ice }}>{fmtPrice(ref.low)}</span></span>
              <span>{tr("ohlcClose")} <span style={{ color: T.ice }}>{fmtPrice(ref.close)}</span></span>
        ...
        {chartLoading || !chartData ? (
          ...
          <TerminalChart key={`${t.id}-${tf}`} candles={chartData.candles} height={340} themeKey={themeKey} onHover={setHovered} />
        )}
      </div>
      ...
      <div className="fx-modal-card" onClick={(e) => e.stopPropagation()} style={{ width: "100%", background: T.surface, border: `1px solid ${T.lineHi}`, borderRadius: "22px 22px 0 0", padding: 20, maxHeight: "88%", overflowY: "auto" }}>
        <div className="flex items-center justify-between" style={{ marginBottom: 14 }}>
          <div className="flex items-center gap-2">
            <TokenAvatar size={34} image={token.image}>{token.emoji}</TokenAvatar>
            <div>
...
function PortfolioTokenCard({ t, onOpen }) {
  ...
  return (
    <button onClick={() => onOpen(t)} className="fx-card w-full flex items-center gap-3 rounded-2xl text-left" style={{ background: T.surface, border: `1px solid ${up ? "rgba(49,208,123,0.28)" : "rgba(255,77,77,0.24)"}`, padding: "12px 14px", position: "relative", overflow: "hidden" }}>
      <TrendFX up={up} seedKey={t.seed} />
      <TokenAvatar tone={up ? "up" : "down"} image={t.image}>{t.emoji}</TokenAvatar>
...
function MyTokenCard({ t, onManage }) {
  return (
    <GlassCard style={{ padding: "12px 14px" }} className="flex items-center gap-3">
      <TokenAvatar tone={t.verified ? "neutral" : "neutral"} image={t.image}>{t.emoji}</TokenAvatar>
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5">
          <span style={{ fontFamily: displayFont, color: T.ice, fontSize: 14, fontWeight: 600 }}>{t.name}</span>
    ...
    <div className="fx-modal-back" style={{ position: "absolute", inset: 0, zIndex: 60, background: "rgba(2,2,4,0.75)", backdropFilter: "blur(4px)", display: "flex", alignItems: "flex-end" }} onClick={onClose}>
      <div className="fx-modal-card" onClick={(e) => e.stopPropagation()} style={{ width: "100%", background: T.surface, border: `1px solid ${T.lineHi}`, borderRadius: "22px 22px 0 0", padding: 22 }}>
        ...
        <div className="flex items-center gap-3" style={{ marginTop: -8, marginBottom: 14 }}>
          <TokenAvatar size={44} image={token.image}>{token.emoji}</TokenAvatar>
          <div>
...
const FEE_PERCENT = 0.01; // 1% комиссии
  ...
  const [token, setToken] = useState(null); // set together with view -> "token" in openToken; never rendered before that
