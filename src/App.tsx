...
async function fetchJson(url) {
  ...
  if (!res.ok) throw new Error(`GeckoTerminal HTTP ${res.status} on ${url}`);
  return res.json();
}
...
async function fetchTonMemePools(limit = 18) {
  ...
  try {
    const json = await fetchJson(`${GT_BASE}/networks/${GT_NETWORK}/trending_pools?page=1&include=base_token,dex`);
  ...
  try {
    const json = await fetchJson(`${GT_BASE}/networks/${GT_NETWORK}/pools?page=1&sort=h24_volume_usd_desc&include=base_token,dex`);
    const shaped = shapePoolRows(json?.data || [], json?.included || [], limit);
    if (shaped.length) { lastFeedErrorMsg = null; return shaped; }
