...
function TerminalChart({ candles, height = 340, themeKey, onHover, tf, valueFmt }) {
  ...
  function draw() {
    ...
    const { startI, endI, min, range, yFor, xFor, bodyW, plotW, padTop, padBottom, drawHeight } = layout;
    ...
    // Live current-price pill — the highlighted price + ticking clock,
    // drawn last so it sits on top of both the candles and the axis.
    if (lastCandle && pillTop != null) {
      ...
      ctx.fillText(timeLabel, widthPx - CHART_GUTTER_W / 2, pillTop + 26);
      ctx.textAlign = "left";
    }
  ...
  // The current-price pill's clock needs a redraw every second even when
  // nothing else about the data has changed, or the time would freeze.
  useEffect(() => {
