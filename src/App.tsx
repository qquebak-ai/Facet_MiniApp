...
function shapePoolRows(rows, included, limit) {
  ...
  return rows.slice(0, limit).map((row) => {
    ...
    return {
      ...
      volNum,
      holders: txns ? txns.buys + txns.sells : 0,
      txCount24h: txns ? txns.buys + txns.sells : 0,
      cat: "Мемы",
  ...
  return (
    <button onClick={() => onOpen(tok)} className="fx-card w-full flex items-center gap-3 rounded-[22px] px-3 py-2.5 text-left" style={{ background: T.surface, border: `1px solid ${T.line}`, animationDelay: `${index * 55}ms` }}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <CardStat icon={User}>{tok.holders.toLocaleString("ru-RU")}</CardStat>
      </div>
        ...
        <div>
          ...
          <button onClick={() => onOpen(spotlight)} className="fx-card w-full flex flex-col items-center text-center gap-2.5 rounded-[22px] p-6" style={{ background: T.surface, border: `1px solid ${T.line}`, position: "relative", overflow: "hidden" }}>
            ...
            <div style={{ position: "relative", zIndex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
              ...
              <CardStat icon={User}>{spotlight.holders.toLocaleString("ru-RU")}</CardStat>
    ...
    <div className="fx-view flex flex-col gap-4 pb-4" style={{ position: "relative" }}>
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex flex-col items-end gap-1" style={{ fontFamily: bodyFont, fontSize: 12, paddingTop: 2 }}>
          <div className="flex items-center gap-2"><span style={{ color: T.muted }}>{tr("statHolders")}</span><span style={{ fontFamily: monoFont, color: T.ice }}>{t.holders.toLocaleString("ru-RU")}</span></div>
          <div className="flex items-center gap-2"><span style={{ color: T.muted }}>{tr("statTransactions")}</span><span style={{ fontFamily: monoFont, color: T.ice }}>{(t.txCount24h || 0).toLocaleString("ru-RU")}</span></div>
      ...
      {tab === "info" && (
        <div className="flex flex-col gap-3">
          <div className="grid grid-cols-2 gap-2">
            ...
            <StatChip icon={User} label={tr("statHolders")} value={t.holders.toLocaleString("ru-RU")} />
