...
async function fetchTonMemePools(limit = 18) {
  try {
    ...
    return rows.map((row) => {
      ...
      // address, a positive traded price, and non-zero liquidity — this is
      // what excludes dead/broken/rugged pools from the feed, so every card
      // in the app corresponds to a token that is actually trading right now.
    }).filter(t => t.poolAddress && t.price > 0 && t.liqNum > 0);
  } catch (err) {
