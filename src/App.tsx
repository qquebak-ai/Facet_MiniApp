...
const SPARK_POSITION_SETS = [
  [12, 30, 50, 70, 88],
  [8, 26, 46, 64, 84],
  [16, 36, 56, 76, 92],
];
function WidgetSparks({ variant = 0 }) {
  const items = useMemo(() => {
    const positions = SPARK_POSITION_SETS[variant % SPARK_POSITION_SETS.length];
    const dur = 4.4;
    return positions.map((left, i) => ({
      left,
      delay: (i / positions.length) * dur,
      dur,
      size: 2.4 + (i % 2) * 1.2,
    }));
  }, [variant]);
