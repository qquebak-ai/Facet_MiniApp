...
async function fetchPoolOHLCV(poolAddress, tf) {
  ...
  try {
    ...
    return {
      ...
      volume: candles.map(c => ({ time: c.time, value: c.volume, color: c.close >= c.open ? hexA(T.up, 0.32) : hexA(T.down, 0.32) })),
    };
  } catch (err) {
...
/* LWChart — real candlestick + volume chart on TradingView's lightweight-charts
   (canvas-based). Pan, pinch-zoom, scroll-zoom and the crosshair with axis
   labels are all native to the library — no hand-rolled gesture code needed,
   and it stays smooth even with hundreds of candles. Colors are read from the
   current theme (T.up/T.down unchanged) and re-applied whenever themeKey
   changes, so a Dark/White toggle re-themes the chart in place. */
const LWChart = React.memo(function LWChart({ candles, volume, height = 230, themeKey }) {
  ...
  useEffect(() => {
    ...
    const chart = createChart(containerRef.current, {
      autoSize: true,
      layout: { background: { type: ColorType.Solid, color: "transparent" }, textColor: T.muted, fontFamily: monoFont, fontSize: 10 },
      grid: { vertLines: { color: "transparent" }, horzLines: { color: T.line } },
      rightPriceScale: { borderColor: T.line },
      timeScale: { borderColor: T.line, timeVisible: true, secondsVisible: false },
      crosshair: {
        mode: CrosshairMode.Normal,
        vertLine: { color: T.ice, width: 1, style: 2, labelBackgroundColor: T.ice },
        horzLine: { color: T.ice, width: 1, style: 2, labelBackgroundColor: T.ice },
      },
  ...
  // Re-theme in place (no remount) whenever the app's Dark/White theme changes.
  useEffect(() => {
    ...
    chartRef.current.applyOptions({
      ...
      crosshair: {
        vertLine: { color: T.ice, labelBackgroundColor: T.ice },
        horzLine: { color: T.ice, labelBackgroundColor: T.ice },
      },
    });
    candleSeriesRef.current?.applyOptions({ upColor: T.up, downColor: T.down, wickUpColor: T.up, wickDownColor: T.down });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [themeKey]);

  // Push new data whenever the candle set changes (new token/timeframe).
  useEffect(() => {
...
function TokenDetail({ t, onBack, showToast, onBuy, onSell, unlocked = true, themeKey }) {
  ...
  // Real OHLCV when the token is backed by a live on-chain pool; a
  // synthetic random-walk chart otherwise (bundled demo tokens, or if
  // the fetch fails — e.g. offline / rate-limited) so the screen never
  // shows a blank chart.
  useEffect(() => {
    let cancelled = false;
  ...
  // Live tick: for real pools, refetch the latest candle every few
  // seconds and push just that one update — lightweight-charts'
  // series.update() repaints only the changed bar, so this stays smooth
  // even on a slow connection. For the synthetic fallback, wiggle the
  // last candle locally like a simulated feed.
  useEffect(() => {
    if (!chartData) return;
    ...
    <div className="fx-view flex flex-col gap-4 pb-4" style={{ position: "relative" }}>
      ...
      <div className="rounded-2xl overflow-hidden" style={{ background: T.surface, border: `1px solid ${T.line}`, padding: "10px 10px 4px" }} key={tf}>
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          <span style={{ fontFamily: monoFont, color: T.muted, fontSize: 10 }}>· {tf} · {chartData?.isLive ? "GeckoTerminal" : "faceta"}</span>
        ...
        {chartLoading || !chartData ? (
          ...
          <LWChart candles={chartData.candles} volume={chartData.volume} themeKey={themeKey} />
        )}
      </div>
...
function CreateView({ showToast, unlocked, accountCreated, connected, onOpenCreateProfile, onOpenConnectModal, onLaunch }) {
  ...
  return (
    <div className="fx-view flex flex-col gap-7 pb-36" style={{ position: "relative" }}>
      ...
      <div className="flex items-center justify-between rounded-xl px-3.5 py-2.5" style={{ background: T.surface, border: `1px solid ${T.line}` }}>
        <span style={{ fontFamily: bodyFont, color: T.muted, fontSize: 12 }}>Общий выпуск (фиксирован)</span>
        <span style={{ fontFamily: monoFont, color: T.ice, fontSize: 13 }}>{TOKEN_FIXED_SUPPLY_LABEL}</span>
      </div>
