    ...
    <div className="fx-view flex flex-col gap-4 pb-4" style={{ position: "relative" }}>
      ...
      <div className="rounded-2xl overflow-hidden" style={{ background: T.surface, border: `1px solid ${T.line}`, padding: "10px 10px 4px" }} key={tf}>
        ...
        {chartLoading || !chartData ? (
          ...
          <TerminalChart candles={chartData.candles} volume={chartData.volume} onHover={setHovered} />
        )}
      </div>
...
function TokenLaunchOverlay({ open, form, category, logoUrl, buyAmount, onClose, onViewToken }) {
  ...
  useEffect(() => {
    ...
    // Purely cosmetic client-side timer sequence — nothing here issues
    // any request. Each stage just advances a local index after a delay.
    const delays = [650, 900, 1050, 750]; // preparing / generating / deploying / confirming
    const timers = [];
    let elapsed = 0;
    delays.forEach((d, i) => {
      elapsed += d;
    ...
    timers.push(setTimeout(() => {
      resultRef.current = {
        name: form.name.trim() || "Unnamed Token",
        ticker: (form.ticker.trim() || "TOKEN").toUpperCase(),
        supply: TOKEN_FIXED_SUPPLY_LABEL,
        buyAmount: buyAmount && buyAmount.trim() ? buyAmount.trim() : "0",
        buyTokens: tokensForTon(parseFloat((buyAmount || "0").replace(",", "."))).tokens,
        buyPct: tokensForTon(parseFloat((buyAmount || "0").replace(",", "."))).pct,
        category: category || null,
        logoUrl: logoUrl || null,
        address: generateFakeContractAddress(),
  ...
  function copyAddr() {
    const addr = resultRef.current && resultRef.current.address;
    ...
    <div className="fx-modal-back" style={{ position: "absolute", inset: 0, zIndex: 80, background: "rgba(2,2,4,0.92)", backdropFilter: "blur(6px)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      {!done ? (
        ...
        <div className="fx-modal-card fx-view flex flex-col items-center text-center gap-4" style={{ width: "100%", maxWidth: 360, background: T.surface, border: `1px solid ${T.lineHi}`, borderRadius: 24, padding: 24 }}>
          <div style={{ width: 68, height: 68, borderRadius: "50%", overflow: "hidden", background: resultRef.current.logoUrl ? `center/cover no-repeat url(${resultRef.current.logoUrl})` : T.surfaceHi, border: `1px solid ${T.lineHi}`, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: `0 0 26px ${glow(0.32)}` }}>
            {!resultRef.current.logoUrl && <Rocket size={26} color={T.electric} />}
          </div>
          ...
          <div>
            <div style={{ fontFamily: displayFont, color: T.ice, fontSize: 19, fontWeight: 700 }}>{resultRef.current.name}</div>
            <div style={{ fontFamily: monoFont, color: T.muted, fontSize: 12, marginTop: 2 }}>${resultRef.current.ticker}</div>
          </div>
          ...
          <div className="w-full flex flex-col gap-2 mt-1">
            <div className="flex items-center justify-between rounded-xl px-3 py-2.5" style={{ background: T.surfaceHi, border: `1px solid ${T.line}` }}>
              ...
              <span style={{ fontFamily: monoFont, fontSize: 12, color: T.ice }}>{resultRef.current.supply}</span>
            </div>
            <div className="flex items-center justify-between rounded-xl px-3 py-2.5" style={{ background: T.surfaceHi, border: `1px solid ${T.line}` }}>
              ...
              <span style={{ fontFamily: monoFont, fontSize: 12, color: T.ice, textAlign: "right" }}>
                {resultRef.current.buyAmount} TON<br />
                <span style={{ fontSize: 10.5, color: T.muted }}>
                  {resultRef.current.buyTokens.toLocaleString("ru-RU")} {resultRef.current.ticker} · {resultRef.current.buyPct.toFixed(resultRef.current.buyPct < 1 ? 3 : 1)}%
                </span>
              </span>
            </div>
            <div className="flex items-center justify-between rounded-xl px-3 py-2.5 gap-2" style={{ background: T.surfaceHi, border: `1px solid ${T.line}` }}>
              <div className="flex flex-col items-start min-w-0">
                ...
                <span style={{ fontFamily: monoFont, fontSize: 11.5, color: T.ice, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: 190 }}>{resultRef.current.address}</span>
              </div>
          ...
          <div className="flex flex-col gap-2 w-full mt-2">
            <button
              onClick={() => onViewToken && onViewToken(resultRef.current)}
            ...
            <button onClick={() => onClose && onClose(resultRef.current)} className="fx-tap w-full rounded-xl py-3" style={{ background: "transparent", border: `1px solid ${T.line}`, fontFamily: bodyFont, fontSize: 13, color: T.muted }}>
              {t("doneClose")}
            </button>
          </div>
        </div>
...
function CreateView({ showToast, unlocked, accountCreated, connected, onOpenCreateProfile, onOpenConnectModal, onLaunch }) {
  ...
  function handleLaunch() {
    ...
    // No blockchain / RPC / smart-contract call happens here — this only
    // asks the root app to open a local, fully simulated creation sequence
    // (see TokenLaunchOverlay, rendered at the app root). Nothing is sent
    // over the network.
    onLaunch({ form, category, logoUrl, buyAmount: form.buyAmount.trim(), onFinish: finishLaunch });
  }
...
const FEE_PERCENT = 0.01; // 1% комиссии
  ...
  const [launchRequest, setLaunchRequest] = useState(null); // { form, category, logoUrl, onFinish } | null
  const EMPTY_LAUNCH_FORM = { name: "", ticker: "", buyAmount: "", desc: "", tg: "", x: "", site: "" };
  function handleLaunchRequest(req) { setLaunchRequest(req); }
