...
function TerminalChart({ candles, height = 340, themeKey, onHover }) {
  ...
  function draw() {
    ...
    // Minimal reference grid — just 3 faint horizontal lines with a price
    // label each, nothing vertical, no clutter competing with the candles.
    ctx.strokeStyle = hexA(T.line, 0.5);
    ctx.lineWidth = 1;
...
function HomeHero({ tokens, onGoTab, onOpen }) {
  const topGainer = useMemo(() => {
    if (!tokens.length) return null;
    return [...tokens].sort((a, b) => b.change - a.change)[0];
  }, [tokens]);
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {topGainer && (
        <button onClick={() => onOpen(topGainer)} className="fx-card w-full flex items-center justify-between gap-3 rounded-2xl p-4 text-left" style={{ background: T.surface, border: `1px solid ${T.line}` }}>
          <div className="flex items-center gap-3 min-w-0">
            <TokenAvatar size={40} tone={topGainer.change >= 0 ? "up" : "down"} src={topGainer.logoUrl}>{topGainer.emoji}</TokenAvatar>
            <div className="min-w-0">
              <div style={{ fontFamily: displayFont, color: T.ice, fontSize: 13.5, fontWeight: 700 }}>{t("homeTopGainer")}</div>
              <div style={{ fontFamily: bodyFont, color: T.muted, fontSize: 11.5 }}>{topGainer.ticker}</div>
            </div>
          </div>
          <div className="flex items-center gap-2 flex-shrink-0">
            <ChangeBadge value={topGainer.change} />
...
function HomeView({ onOpen, tokens, loading, onGoTab }) {
  ...
  return (
    <div className="flex flex-col" style={{ height: "100%", minHeight: 0 }}>
      <div className="flex flex-col gap-4 flex-shrink-0" style={{ paddingBottom: 12 }}>
        <HomeHero tokens={tokens} onGoTab={onGoTab} onOpen={onOpen} />
    ...
    <div style={{ background: T.bg, height, minHeight: height, width: "100%", maxWidth: 480, margin: "0 auto", fontFamily: bodyFont, position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ position: "relative", zIndex: 1, height: "100%", display: "flex", flexDirection: "column" }}>
        ...
        <div className="no-scrollbar px-4" style={{ flex: 1, overflowY: view === "home" ? "hidden" : "auto", minHeight: 0 }} key={view}>
          {view === "home" && <HomeView onOpen={openToken} tokens={tokens} loading={tokensLoading} onGoTab={goTab} />}
