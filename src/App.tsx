...
function TokenLaunchOverlay({ open, form, category, logoUrl, onClose, onViewToken }) {
  ...
  useEffect(() => {
    ...
    timers.push(setTimeout(() => {
      resultRef.current = {
        ...
        supply: form.supply.trim() || "1,000,000,000",
...
function CreateView({ showToast, unlocked, accountCreated, connected, onOpenCreateProfile, onOpenConnectModal, onTokenCreated, onGoProfile }) {
  ...
  function handleLaunch() {
    ...
    // opens a local, fully simulated creation sequence (see
    // TokenLaunchOverlay). Nothing is sent over the network.
    setLaunching(true);
  }

  function resetForm() {
    setForm({ name: "", ticker: "", supply: "", desc: "", tg: "", x: "", site: "" });
    setCategory(null);
    setLogoUrl(null);
    setBannerUrl(null);
  }

  function finishLaunch(result, alsoView) {
    setLaunching(false);
    if (result && onTokenCreated) onTokenCreated(result);
    if (result) showToast(`Токен ${result.name} ($${result.ticker}) создан ✅`);
    resetForm();
    if (alsoView && onGoProfile) onGoProfile();
  }
  ...
  return (
    <div className="fx-view flex flex-col gap-7 pb-36" style={{ position: "relative" }}>
      ...
      <TokenLaunchOverlay
        ...
        category={category}
        logoUrl={logoUrl}
        onClose={(result) => finishLaunch(result, false)}
        onViewToken={(result) => finishLaunch(result, true)}
      />
    </div>
  );
}
