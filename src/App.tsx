...
function Field({ label, placeholder, area, value, onChange }) {
  const Comp = area ? "textarea" : "input";
  const [focus, setFocus] = useState(false);
  return (
    <label className="flex flex-col gap-1.5">
      <span style={{ fontFamily: bodyFont, color: T.muted, fontSize: 12 }}>{label}</span>
      <Comp placeholder={placeholder} rows={area ? 3 : undefined} value={value} onChange={onChange} onFocus={() => setFocus(true)} onBlur={() => setFocus(false)}
        className="rounded-xl px-3 py-2.5 w-full"
        style={{ fontFamily: bodyFont, fontSize: 13, color: T.ice, background: T.surface, border: `1px solid ${focus ? T.electric : T.line}`, outline: "none", resize: area ? "none" : undefined, boxShadow: focus ? `0 0 0 3px rgba(255,255,255,0.14)` : "none", transition: `border-color ${EASE}, box-shadow ${EASE}` }} />
    </label>
  );
}
...
function WalletCard({ connected, onConnect, onDisconnect, onCopy, onExplore }) {
  const [copied, setCopied] = useState(false);
  const balance = useCountUp(connected ? 128.4 : 0, 900, connected);
  const usd = useCountUp(connected ? 910 : 0, 900, connected);
  ...
  return (
    <GlassCard style={{ padding: 20 }} className="fx-view">
      ...
      <div className="flex items-center gap-2 mt-3 rounded-xl px-3 py-2" style={{ background: T.surface, border: `1px solid ${T.line}` }}>
        <span style={{ fontFamily: monoFont, color: T.ice, fontSize: 12, flex: 1 }}>EQA8...AB12</span>
...
/* CreateAccountModal — the real signup form behind "Создать аккаунт":
   nickname (English/Latin only) is the single required field; avatar,
   email and bio are all optional. No avatar → a random emoji becomes
   the profile picture instead. */
function CreateAccountModal({ open, onClose, onSubmit, initial, mode = "create" }) {
  ...
  const canSubmit = nicknameValid && emailValid;
    ...
    <div className="fx-modal-back" style={{ position: "absolute", inset: 0, zIndex: 60, background: "rgba(2,2,4,0.75)", backdropFilter: "blur(4px)", display: "flex", alignItems: "flex-end" }} onClick={onClose}>
      <div className="fx-modal-card" onClick={(e) => e.stopPropagation()} style={{ width: "100%", background: T.surface, border: `1px solid ${T.lineHi}`, borderRadius: "22px 22px 0 0", padding: 22, maxHeight: "88%", overflowY: "auto" }}>
        <div className="flex items-center justify-between" style={{ marginBottom: 4 }}>
          <span style={{ fontFamily: displayFont, color: T.ice, fontSize: 16, fontWeight: 700 }}>{isEdit ? "Редактировать профиль" : "Создать аккаунт"}</span>
          <button onClick={onClose} className="fx-tap"><X size={16} color={T.muted} /></button>
        </div>
        <p style={{ fontFamily: bodyFont, color: T.muted, fontSize: 12, marginBottom: 16 }}>Никнейм и почта обязательны, остальное можно заполнить позже.</p>
        ...
        <div className="flex flex-col gap-3.5">
          ...
          <Field label="Почта *" placeholder="you@example.com" value={email} onChange={(e) => setEmail(e.target.value)} />
...
function ProfileView({
  connected, onConnect, onDisconnect, onOpenConnectModal, showToast,
  accountCreated, profile, onOpenCreateProfile, onOpenEditProfile, onLogOut,
  ...
  function copyAddress() {
    if (typeof navigator !== "undefined" && navigator.clipboard) navigator.clipboard.writeText("EQA8...AB12").catch(() => {});
        ...
        <div className="flex flex-col items-center text-center gap-2" style={{ marginTop: 10, position: "relative" }}>
          ...
          {accountCreated ? (
            ...
            <>
              ...
              <p style={{ fontFamily: bodyFont, color: T.muted, fontSize: 12.5, maxWidth: 260, lineHeight: 1.5 }}>Создай аккаунт, чтобы запускать токены, торговать и собирать достижения.</p>
              <button onClick={onOpenCreateProfile} className="fx-tap rounded-xl px-5 py-2.5 mt-2" style={{ background: PRISM, color: "#08080A", fontFamily: displayFont, fontWeight: 700, fontSize: 13 }}>Создать аккаунт</button>
            </>
          )}
        </div>

        <div className="mt-5"><WalletCard connected={connected} onConnect={connectWallet} onDisconnect={disconnectWallet} onCopy={copyAddress} onExplore={exploreWallet} /></div>
...
export default function TonLaunchApp() {
  ...
  // CreateAccountModal bottom sheet can be rendered as a direct child of
  ...
  function submitProfile(data) {
    ...
    showToast(profileModalMode === "edit" ? "Профиль обновлён" : "Аккаунт создан");
  }
  function logOutProfile() {
    setAccountCreated(false);
    setProfile({ nickname: "", email: "", bio: "", avatarUrl: null, emoji: null });
    setConnected(false);
  ...
  function confirmTrade(mode, payAmount, receiveAmount, unit) {
    setTradeModal(null);
    if (mode === "buy") showToast(`Куплено ≈ ${receiveAmount} $${token.ticker} за ${payAmount} ${unit} 🚀`);
    else showToast(`Продано ${payAmount} $${token.ticker} за ≈ ${receiveAmount} ${unit} 💸`);
  }
  ...
  return (
    <div style={{ background: T.bg, height, minHeight: height, width: "100%", maxWidth: 480, margin: "0 auto", fontFamily: bodyFont, position: "relative", overflow: "hidden" }}>
      ...
      <ConnectModal open={connectModalOpen} onClose={() => setConnectModalOpen(false)} onConnect={() => setConnected(true)} />
      ...
      <SettingsPanel
        ...
        onDisconnectWallet={() => setConnected(false)}
        onCopyAddress={() => {
          if (typeof navigator !== "undefined" && navigator.clipboard) navigator.clipboard.writeText("EQA8...AB12").catch(() => {});
          showToast("Адрес скопирован");
        }}
      ...
      <div style={{ position: "relative", zIndex: 1, height: "100%", display: "flex", flexDirection: "column" }}>
        <div className="flex items-center justify-between px-4 pt-4 pb-2" style={{ flexShrink: 0 }}>
          ...
          <button onClick={handleHeaderWalletClick} className="fx-tap flex items-center gap-1.5 rounded-full px-3 py-1.5" style={{ background: T.surface, border: `1px solid ${connected ? "rgba(255,255,255,0.35)" : T.line}` }}>
            ...
            <span style={{ fontFamily: monoFont, color: connected ? T.turquoise : T.ice, fontSize: 11 }}>{connected ? "EQA8...AB12" : "Connect"}</span>
          </button>
        </div>
