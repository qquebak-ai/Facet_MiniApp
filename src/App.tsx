...
const FEE_PERCENT = 0.01; // 1% комиссии
  ...
  async function clearAllMyTokens() {
    ...
    setMyTokens([]);
    setCommunityTokens((prev) => prev.filter((tok) => !ids.includes(tok.id)));
    setHoldings({});
    ...
    if (error) { console.error("[mintly] failed to clear tokens in Supabase:", error); showToast(t("deleteFailedToast")); if (userId) loadMyTokens(userId); loadCommunityTokens(); }
  }
