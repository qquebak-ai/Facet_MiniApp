...
const STR = {
  RU: {
    ...
    pinRow: "PIN-код",
    pinRowSub: "Запрашивать код при каждом открытии Faceta",
    ...
    privacyText: "Мы собираем только данные, необходимые для работы приложения: никнейм, адрес кошелька и историю сделок внутри Faceta. Данные не передаются третьим лицам в рекламных целях. Ты можешь удалить аккаунт в любой момент — все локальные данные профиля будут стёрты немедленно.",
    termsText: "Используя Faceta, ты подтверждаешь, что совершаешь сделки на свой риск. Faceta не гарантирует доходность токенов и не несёт ответственности за потери, вызванные волатильностью рынка. Запрещено создавать токены, вводящие пользователей в заблуждение, или использующие чужой бренд без разрешения.",
  ...
  EN: {
    ...
    pinRow: "PIN code",
    pinRowSub: "Require a code every time Faceta opens",
    ...
    copyLink: "Copy link",
    privacyText: "We only collect data needed to run the app: nickname, wallet address, and your trade history within Faceta. Data is never shared with third parties for advertising. You can delete your account at any time — all local profile data is erased immediately.",
    termsText: "By using Faceta, you confirm you trade at your own risk. Faceta does not guarantee token returns and is not liable for losses caused by market volatility. Creating tokens that mislead users or use someone else's brand without permission is prohibited.",
...
async function fetchTonMemePools(limit = 18) {
  try {
    ...
    // `included` holds the actual token records (name/symbol/image) that
    // the pool rows reference by relationship id — build a lookup so each
    // pool can pull its base token's real logo + full project name.
    const tokenById = {};
    for (const inc of json?.included || []) {
      if (inc.type === "token") tokenById[inc.id] = inc.attributes || {};
    }
    return rows.map((row) => {
      const a = row.attributes || {};
      const baseTokenId = row.relationships?.base_token?.data?.id;
      const baseToken = baseTokenId ? tokenById[baseTokenId] : null;
      const ticker = (baseToken?.symbol || (a.name || "TOKEN/TON").split("/")[0].trim()).toUpperCase().slice(0, 10) || "TOKEN";
      const fullName = (baseToken?.name || ticker).trim();
      const rawImg = baseToken?.image_url;
      const imgUrl = rawImg && !rawImg.includes("missing.png") ? rawImg : null;
...
// Fetches a real pool's description + social links (GeckoTerminal's
// separate "pool info" endpoint — trending_pools doesn't include these).
// Used lazily, only for the token the person actually opens, so we don't
// burn extra requests on tokens nobody looks at.
async function fetchPoolInfo(poolAddress) {
  try {
    const res = await fetch(`${GT_BASE}/networks/${GT_NETWORK}/pools/${poolAddress}/info`);
    if (!res.ok) throw new Error(`GeckoTerminal ${res.status}`);
    const json = await res.json();
    ...
    return {
      ...
      imgUrl: a.image_url && !a.image_url.includes("missing.png") ? a.image_url : null,
    };
  } catch (err) {
    return null;
  }
...
// TerminalChart — our own candlestick + volume renderer on <canvas>, no
// external chart library and no third-party branding/watermark. Used for
// both real (GeckoTerminal-sourced) and synthetic demo data — we just draw
// whatever candles/volume we're given. Handles resize via ResizeObserver,
// crisp rendering via devicePixelRatio, and a touch/mouse crosshair that
// reports the hovered candle's OHLC back through onHover.
function TerminalChart({ candles, volume, height = 260, onHover }) {
  ...
  const layout = useMemo(() => {
    ...
    if (!n) return null;
    const volH = volume?.length ? height * 0.2 : 0;
    const priceH = height - volH;
    const padTop = priceH * 0.06, padBottom = priceH * 0.06;
    const highs = candles.map(c => c.high), lows = candles.map(c => c.low);
    const max = Math.max(...highs), min = Math.min(...lows);
    const range = (max - min) || (max * 0.02) || 1;
    const slot = widthPx / n;
    const bodyW = Math.max(1.5, Math.min(10, slot * 0.62));
    const maxVol = volume?.length ? Math.max(...volume.map(v => v.value || 0), 1) : 1;
    const yFor = (price) => padTop + (1 - (price - min) / range) * (priceH - padTop - padBottom);
    const xFor = (i) => i * slot + slot / 2;
    return { n, volH, priceH, min, max, range, slot, bodyW, maxVol, yFor, xFor };
  }, [candles, volume, widthPx, height]);

  // Draw whenever data, size, or theme (T mutates in place, themeKey drives re-render upstream) changes.
  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas || !layout || !widthPx) return;
    ...
    const { n, priceH, min, range, slot, bodyW, maxVol, volH, yFor, xFor } = layout;
    ...
    // volume
    if (volume?.length && volH) {
      const volTop = priceH;
      for (let i = 0; i < volume.length; i++) {
        const v = volume[i];
        const x = xFor(i);
        const h = Math.max(1, (v.value / maxVol) * (volH * 0.9));
        const up = candles[i] ? candles[i].close >= candles[i].open : true;
        ctx.fillStyle = v.color || (up ? hexA(T.up, 0.32) : hexA(T.down, 0.32));
    ...
    // crosshair
    if (hoverX != null) {
      const i = Math.max(0, Math.min(n - 1, Math.floor(hoverX / slot)));
      const x = xFor(i);
      ...
      ctx.stroke();
      ctx.setLineDash([]);
      onHover && onHover(candles[i]);
    } else {
      onHover && onHover(null);
    }
  }, [layout, widthPx, height, hoverX, candles, volume, onHover]);
  ...
  function handleLeave() { setHoverX(null); }

  if (!candles?.length) return null;
  return (
    <div ref={wrapRef} style={{ width: "100%", height, position: "relative", touchAction: "pan-y" }}
  ...
  return (
    <div className="fx-avatar" style={{ width: size, height: size, position: "relative", flexShrink: 0 }}>
      ...
      <div style={{
        ...
        {showImg
          ? <img src={src} alt="" loading="lazy" onError={() => setImgFailed(true)} style={{ width: "100%", height: "100%", objectFit: "cover", borderRadius: "50%" }} />
          : children}
      </div>
    </div>
  );
    ...
    <button onClick={() => onOpen(t)} className="fx-card w-full text-left rounded-2xl" style={{ background: T.surface, border: `1px solid ${up ? "rgba(49,208,123,0.28)" : "rgba(255,77,77,0.24)"}`, padding: "9px 12px", animationDelay: `${index * 55}ms`, position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ position: "relative", overflow: "hidden", borderRadius: 10 }}>
        ...
        <div className="flex items-center gap-2.5" style={{ position: "relative", zIndex: 1 }}>
          <TokenAvatar size={42} tone={up ? "up" : "down"} src={t.imgUrl}>{t.emoji}</TokenAvatar>
...
function TokenDetail({ t, onBack, showToast, onBuy, onSell, unlocked = true, themeKey }) {
  ...
  const [poolInfo, setPoolInfo] = useState(null); // { description, website, twitter, telegram, imgUrl } | null
  const up = t.change >= 0;

  // Real description + social links for live pools — fetched lazily
  // (only for the token actually opened) from GeckoTerminal's pool-info
  // endpoint, which trending_pools doesn't include.
  useEffect(() => {
    setPoolInfo(null);
    if (!t.poolAddress) return;
    let cancelled = false;
    fetchPoolInfo(t.poolAddress).then(info => { if (!cancelled) setPoolInfo(info); });
    return () => { cancelled = true; };
  }, [t.poolAddress]);
  ...
  function openSocial(kind) {
    let url = null;
    if (poolInfo) {
      url = kind === "tg" ? poolInfo.telegram : kind === "x" ? poolInfo.twitter : poolInfo.website;
    }
    if (!url) {
      const handle = DETAIL_EXTRA.creator.replace("@", "");
      const fallback = { tg: `https://t.me/${handle}`, x: `https://x.com/${handle}`, site: `https://${t.ticker.toLowerCase()}.xyz` };
      url = fallback[kind];
    }
  ...
  return (
    <div className="fx-view flex flex-col gap-4 pb-4" style={{ position: "relative" }}>
      ...
      <div className="flex items-center gap-3">
        <TokenAvatar size={56} tone={up ? "up" : "down"} src={t.imgUrl}>{t.emoji}</TokenAvatar>
        <div>
          <div className="flex items-center gap-1.5">
            <span style={{ fontFamily: displayFont, color: T.ice, fontSize: 19, fontWeight: 700 }}>{t.name}</span>
            {(t.poolAddress ? t.verified : DETAIL_EXTRA.verified) && <ShieldCheck size={14} color={T.electric} />}
          </div>
      ...
      <div className="rounded-2xl overflow-hidden" style={{ background: T.surface, border: `1px solid ${T.line}`, padding: "10px 10px 4px" }} key={tf}>
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          {chartData?.isLive && <span style={{ width: 5, height: 5, borderRadius: "50%", background: T.up, boxShadow: `0 0 5px ${T.up}` }} />}
        ...
        {chartLoading || !chartData ? (
          <div className="flex items-center justify-center" style={{ height: 230, fontFamily: monoFont, fontSize: 11, color: T.muted }}>
            {tr("chartLoading")}
          </div>
        ) : (
          <TerminalChart key={themeKey} candles={chartData.candles} volume={chartData.volume} onHover={setHovered} />
        )}
      </div>
      ...
      <div className="rounded-2xl p-4" style={{ background: T.surface, border: `1px solid ${T.line}` }}>
        ...
        <p style={{ fontFamily: bodyFont, color: T.muted, fontSize: 13, lineHeight: 1.5 }}>{(t.poolAddress ? poolInfo?.description : null) || tr("mockTokenDesc")}</p>
        <div className="flex items-center gap-4 mt-3">
          <button onClick={() => openSocial("tg")} className="fx-tap" style={{ opacity: t.poolAddress && !poolInfo?.telegram ? 0.35 : 1 }}><Send size={15} color={T.muted} /></button>
          <button onClick={() => openSocial("x")} className="fx-tap" style={{ opacity: t.poolAddress && !poolInfo?.twitter ? 0.35 : 1 }}><Twitter size={15} color={T.muted} /></button>
          <button onClick={() => openSocial("site")} className="fx-tap" style={{ opacity: t.poolAddress && !poolInfo?.website ? 0.35 : 1 }}><Globe size={15} color={T.muted} /></button>
          {!t.poolAddress && <span style={{ fontFamily: bodyFont, color: T.muted, fontSize: 12, marginLeft: "auto" }}>{tr("creatorLabel")} {DETAIL_EXTRA.creator}</span>}
        </div>
      </div>
      </div>
    </div>
      ...
      <div className="fx-modal-card" onClick={(e) => e.stopPropagation()} style={{ width: "100%", background: T.surface, border: `1px solid ${T.lineHi}`, borderRadius: "22px 22px 0 0", padding: 20, maxHeight: "88%", overflowY: "auto" }}>
        <div className="flex items-center justify-between" style={{ marginBottom: 14 }}>
          <div className="flex items-center gap-2">
            <TokenAvatar size={34} src={token.imgUrl}>{token.emoji}</TokenAvatar>
            <div>
...
function PortfolioTokenCard({ t, onOpen }) {
  ...
  return (
    <button onClick={() => onOpen(t)} className="fx-card w-full flex items-center gap-3 rounded-2xl text-left" style={{ background: T.surface, border: `1px solid ${up ? "rgba(49,208,123,0.28)" : "rgba(255,77,77,0.24)"}`, padding: "12px 14px", position: "relative", overflow: "hidden" }}>
      <TrendFX up={up} seedKey={t.seed} />
      <TokenAvatar tone={up ? "up" : "down"} src={t.imgUrl}>{t.emoji}</TokenAvatar>
...
function MyTokenCard({ t, onManage }) {
  return (
    <GlassCard style={{ padding: "12px 14px" }} className="flex items-center gap-3">
      <TokenAvatar tone={t.verified ? "neutral" : "neutral"} src={t.imgUrl}>{t.emoji}</TokenAvatar>
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5">
          <span style={{ fontFamily: displayFont, color: T.ice, fontSize: 14, fontWeight: 600 }}>{t.name}</span>
    ...
    <div className="fx-modal-back" style={{ position: "absolute", inset: 0, zIndex: 60, background: "rgba(2,2,4,0.75)", backdropFilter: "blur(4px)", display: "flex", alignItems: "flex-end" }} onClick={onClose}>
      <div className="fx-modal-card" onClick={(e) => e.stopPropagation()} style={{ width: "100%", background: T.surface, border: `1px solid ${T.lineHi}`, borderRadius: "22px 22px 0 0", padding: 22 }}>
        ...
        <div className="flex items-center gap-3" style={{ marginTop: -8, marginBottom: 14 }}>
          <TokenAvatar size={44} src={token.imgUrl}>{token.emoji}</TokenAvatar>
          <div>
...
const FEE_PERCENT = 0.01; // 1% комиссии
  ...
  const [myTokens, setMyTokens] = useState(() => {
    try {
      if (typeof window !== "undefined") {
        const saved = window.localStorage.getItem("faceta_my_tokens");
  ...
  const [holdings, setHoldings] = useState(() => {
    try {
      if (typeof window !== "undefined") {
        const saved = window.localStorage.getItem("faceta_holdings");
        if (saved) return JSON.parse(saved);
      }
    } catch (e) { /* localStorage unavailable */ }
    return {};
  ...
  function adjustHolding(tokenId, delta) {
    setHoldings((prev) => {
      ...
      try { if (typeof window !== "undefined") window.localStorage.setItem("faceta_holdings", JSON.stringify(next)); } catch (e) { /* localStorage unavailable */ }
      return next;
    });
  }
  ...
  function handleTokenCreated(result) {
    ...
    setMyTokens((prev) => {
      ...
      try {
        if (typeof window !== "undefined") window.localStorage.setItem("faceta_my_tokens", JSON.stringify(next));
      } catch (e) { /* localStorage unavailable */ }
      return next;
    });
  }
