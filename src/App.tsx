...
const WHITE_THEME = {
  ...
  surfaceHi: "rgba(240,239,244,0.75)",
...
const STR = {
  RU: {
    ...
    liqShort: "Ликв", volShort: "Vol", holdersShort: "держателей",
  ...
  EN: {
    ...
    liqShort: "Liq", volShort: "Vol", holdersShort: "holders",
  ...
  return (
    <style>{`
      ...
      * { -webkit-tap-highlight-color: transparent; }
      ...
      .fx-card { animation: fadeInUp 480ms cubic-bezier(0.16,1,0.3,1) both; transition: transform ${SPRING}, border-color ${EASE}, box-shadow ${EASE}; will-change: transform; backdrop-filter: blur(20px) saturate(160%); -webkit-backdrop-filter: blur(20px) saturate(160%); }
      ...
      .fx-chip { transition: border-color ${EASE}, background ${EASE}, color ${EASE}, transform ${SPRING}; will-change: transform; backdrop-filter: blur(16px) saturate(160%); -webkit-backdrop-filter: blur(16px) saturate(160%); }
...
function TerminalChart({ candles, height = 340, themeKey, onHover, tf }) {
  ...
  const dragRef = useRef(null);     // { lastX, lastT, vx, moved, startX, startY }
  ...
  function clampView() {
    ...
    v.start = Math.max(0, Math.min(n - v.count, v.start));
  }
  ...
  function computeLayout() {
    ...
    const yFor = (price) => padTop + (1 - (price - min) / range) * (height - padTop - padBottom);
    const xFor = (idx) => (idx - v.start + 0.5) * slot;
    return { startI, endI, min, max, range, slot, bodyW, yFor, xFor };
  }
  ...
  function draw() {
    ...
    const { startI, endI, min, range, yFor, xFor, bodyW } = layout;

    // Reference price labels only — no horizontal grid lines cluttering
    // the candles; just the 3 price levels printed faintly on the left.
    ctx.font = "10px " + monoFont;
    ctx.fillStyle = T.muted;
    for (let i = 0; i <= 2; i++) {
      const price = min + (range * i) / 2;
      const y = yFor(price);
      ctx.fillText(fmtPrice(price), 4, Math.max(9, y - 3));
    }

    // Candles — high-contrast bodies + wicks, sized to fill the available
    // width so they stay readable at any zoom level.
    for (let i = startI; i < endI; i++) {
      const c = candles[i];
      if (!c) continue;
      const x = xFor(i);
      if (x < -bodyW || x > widthPx + bodyW) continue;
    ...
    // Last-price line — a dashed horizontal line at the most recent close,
    // with a filled tag on the right showing the exact price, colored by
    // whether that last candle closed up or down.
    ...
    if (lastCandle) {
      ...
      const label = fmtPrice(lastCandle.close);
      ctx.font = "10px " + monoFont;
      const labelW = ctx.measureText(label).width + 10;
      const labelH = 16;
      const labelY = Math.min(Math.max(y - labelH / 2, 0), height - labelH);
      ctx.fillStyle = lastColor;
      ctx.fillRect(widthPx - labelW, labelY, labelW, labelH);
      ctx.fillStyle = T.bg;
      ctx.fillText(label, widthPx - labelW + 5, labelY + labelH - 5);

      // Bar-close timer — countdown to when the current (rightmost) bar
      // closes, based on the timeframe's bar duration. Ticks every second
      // via the interval effect below; hidden once the bar rolls over
      // since a live tick will bring in a fresh candle.
      const barSec = TF_SECONDS[tf] || 3600;
      const closeAtMs = (lastCandle.time + barSec) * 1000;
      const remainMs = closeAtMs - Date.now();
      if (remainMs > 0) {
        const timerLabel = fmtCountdown(remainMs);
        ctx.font = "10px " + monoFont;
        const tw = ctx.measureText(timerLabel).width + 10;
        const th = 16;
        ctx.fillStyle = hexA("#000000", 0.55);
        ctx.fillRect(widthPx - tw, 2, tw, th);
        ctx.fillStyle = T.ice;
        ctx.fillText(timerLabel, widthPx - tw + 5, 2 + th - 5);
      }
    }
  ...
  // The bar-close timer needs a redraw every second even when nothing
  // else about the data has changed, or the countdown text would freeze.
  useEffect(() => {
  ...
  function onTouchStart(e) {
    ...
    dragRef.current = { lastX: x, lastT: performance.now(), vx: 0, moved: false, startX: x, startY: y };
  }
  ...
  function onMouseDown(e) {
    cancelInertia();
    dragRef.current = { lastX: e.clientX, lastT: performance.now(), vx: 0, moved: false, startX: e.clientX, startY: e.clientY };
  }
  function onMouseMove(e) {
    if (!dragRef.current || e.buttons !== 1) return;
    const totalDx = e.clientX - dragRef.current.startX;
    if (!dragRef.current.moved && Math.abs(totalDx) > 4) { dragRef.current.moved = true; hoverIdxRef.current = null; onHover && onHover(null); }
    if (dragRef.current.moved) {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="fx-card rounded-2xl p-5 relative overflow-hidden" style={{ background: T.surface, border: `1px solid ${T.line}` }}>
        ...
        <div className="relative" style={{ fontFamily: bodyFont, color: T.muted, fontSize: 12.5, marginTop: 7, lineHeight: 1.55, maxWidth: 280 }}>
      ...
      <div className="flex items-center justify-around">
        {actions.map(a => (
          <button key={a.key} onClick={a.onClick} className="fx-tap flex flex-col items-center gap-2">
            <div className="fx-card flex items-center justify-center" style={{ width: 56, height: 56, borderRadius: "50%", background: T.surface, border: `1px solid ${T.line}` }}>
              <a.icon size={22} color={T.ice} />
            </div>
            <span style={{ fontFamily: bodyFont, fontSize: 11.5, color: T.muted, textAlign: "center" }}>{t(a.key)}</span>
          </button>
        ))}
      </div>
    </div>
...
const MEMPAD_FILTERS = [
  ...
  { id: "bluming", labelKey: "mempadFilterBluming" },
  { id: "dex", labelKey: "mempadFilterDex" },
];

function MempadRow({ t: tok, onOpen, index }) {
  return (
    <button onClick={() => onOpen(tok)} className="fx-card w-full flex items-center gap-3 rounded-2xl px-3 py-2.5 text-left" style={{ background: T.surface, border: `1px solid ${T.line}`, animationDelay: `${index * 55}ms` }}>
...
function MempadRowSkeleton({ index }) {
  return (
    <div className="fx-card w-full flex items-center gap-3 rounded-2xl px-3 py-2.5" style={{ background: T.surface, border: `1px solid ${T.line}`, animationDelay: `${index * 55}ms` }}>
...
function MempadView({ tokens, loading, onOpen, onLaunch }) {
  ...
  const list = useMemo(() => {
    ...
    switch (filter) {
      ...
      case "bluming": arr = [...arr].sort((a, b) => b.mcapNum - a.mcapNum); break;
      case "dex": arr = arr.filter(tok => tok.verified); break;
      default: arr = [...arr].reverse(); break; // new
    }
    return arr;
  }, [tokens, filter, spotlight]);

  return (
    <div className="flex flex-col gap-5" style={{ paddingBottom: 12 }}>
      <div className="flex items-center justify-between">
        <span style={{ fontFamily: displayFont, color: T.ice, fontSize: 26, fontWeight: 800, letterSpacing: "-0.02em" }}>{t("navMempad")}</span>
      ...
      {loading && !spotlight ? (
        <div className="fx-card rounded-2xl p-6 flex flex-col items-center gap-3" style={{ background: T.surface, border: `1px solid ${T.line}` }}>
        ...
        <div>
          ...
          <button onClick={() => onOpen(spotlight)} className="fx-card w-full flex flex-col items-center text-center gap-2.5 rounded-2xl p-6" style={{ background: T.surface, border: `1px solid ${T.line}`, position: "relative", overflow: "hidden" }}>
  ...
  return (
    <div className="fx-view flex flex-col gap-4 pb-4" style={{ position: "relative" }}>
      ...
      {tab === "chart" && (
        <>
          <div className="flex items-center justify-between gap-2 flex-wrap">
            <div className="flex gap-1 rounded-lg p-0.5" style={{ background: T.surface, border: `1px solid ${T.line}` }}>
              {[["mcap", tr("chartModeMcap")], ["price", tr("chartModePrice")]].map(([id, label]) => (
                <button key={id} onClick={() => setChartMode(id)} className="fx-tap rounded-md px-2.5 py-1" style={{ fontFamily: bodyFont, fontSize: 11.5, fontWeight: 600, background: chartMode === id ? T.ice : "transparent", color: chartMode === id ? T.bg : T.muted }}>{label}</button>
              ))}
            </div>
            <div className="no-scrollbar flex gap-1.5 overflow-x-auto" style={{ flex: 1, justifyContent: "flex-end" }}>
              {(tfExpanded ? TIMEFRAMES : TIMEFRAMES.slice(0, 4)).map(f => (
                <button key={f} onClick={() => changeTf(f)} className="tf-btn fx-tap rounded-lg px-2.5 py-1 flex-shrink-0"
              ...
              <button onClick={() => setTfExpanded(v => !v)} className="fx-tap rounded-lg p-1 flex-shrink-0" style={{ background: T.surface, border: `1px solid ${T.line}` }}>
                <ChevronDown size={14} color={T.muted} style={{ transform: tfExpanded ? "rotate(180deg)" : "none", transition: `transform ${EASE}` }} />
              </button>
            </div>
          </div>

          <div className="rounded-2xl overflow-hidden" style={{ background: T.surface, border: `1px solid ${T.line}`, padding: "8px 8px 2px" }}>
            {chartData && scaledCandles && (() => {
              const ref = hovered || scaledCandles[scaledCandles.length - 1];
              return (
                <div className="flex items-center justify-between flex-wrap" style={{ fontFamily: monoFont, fontSize: 11, color: T.muted, marginBottom: 4, padding: "0 2px" }}>
                  <span>{tr("ohlcClose")}: <span style={{ color: T.ice }}>{chartMode === "price" ? fmtPrice(ref.close) : fmtUSD(ref.close)}</span> &nbsp; <span style={{ color: ref.close >= ref.open ? T.up : T.down }}>{(ref.close - ref.open >= 0 ? "+" : "")}{(ref.close - ref.open).toFixed(2)} ({ref.open ? (((ref.close - ref.open) / ref.open) * 100).toFixed(2) : "0.00"}%)</span></span>
                  <span>{fmtCandleStamp(ref.time)}</span>
                </div>
              );
            })()}
            {chartLoading || !chartData ? (
              ...
              <TerminalChart key={`${t.id}-${tf}-${chartMode}`} candles={scaledCandles} height={340} themeKey={themeKey} onHover={setHovered} tf={tf} />
            )}
          </div>
          ...
          {connected ? (
            <div className="flex gap-2">
              <button onClick={onBuy} className="fx-tap flex-1 rounded-xl py-3 flex items-center justify-center gap-1.5" style={{ fontFamily: displayFont, fontWeight: 700, fontSize: 14, background: PRISM, color: PRISM_TEXT, boxShadow: `0 0 20px ${glow(0.3)}`, opacity: unlocked ? 1 : 0.55 }}>{!unlocked && <Lock size={13} />}{tr("buy")}</button>
            ...
            <button onClick={onConnectWallet} className="fx-tap w-full rounded-xl py-3.5 flex items-center justify-center gap-2" style={{ fontFamily: displayFont, fontWeight: 700, fontSize: 14.5, background: T.ice, color: T.bg }}>
      ...
      {tab === "info" && (
        <div className="flex flex-col gap-3">
          ...
          {(info?.description || info?.telegram || info?.twitter || info?.website) ? (
            ...
            <div className="rounded-2xl p-4 flex items-center justify-center text-center" style={{ background: T.surface, border: `1px dashed ${T.line}`, minHeight: 80 }}>
      ...
      {tab === "tx" && (
        <div className="flex flex-col gap-1.5">
          {!t.poolAddress || (!tradesLoading && trades && trades.length === 0) || (!tradesLoading && !trades) ? (
            ...
            trades.map(tx => (
              <div key={tx.id} className="fx-chip flex items-center justify-between rounded-xl px-3 py-2" style={{ background: T.surface, border: `1px solid ${T.line}` }}>
...
function TokenLaunchOverlay({ open, form, category, logoUrl, buyAmount, stepIndex, done, error, result, onClose, onRetry, onViewToken }) {
  ...
  return (
    <div className="fx-modal-back" style={{ position: "absolute", inset: 0, zIndex: 80, background: "rgba(2,2,4,0.92)", backdropFilter: "blur(6px)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      {error ? (
        <div className="fx-modal-card flex flex-col items-center text-center gap-4" style={{ width: "100%", maxWidth: 340, background: T.surface, border: `1px solid ${T.lineHi}`, borderRadius: 24, padding: 24 }}>
          ...
          <div className="flex flex-col gap-2 w-full mt-1">
            <button onClick={onRetry} className="fx-tap w-full rounded-xl py-3" style={{ background: PRISM, color: PRISM_TEXT, fontFamily: displayFont, fontWeight: 700, fontSize: 14 }}>
              {t("retry")}
            </button>
            <button onClick={() => onClose && onClose(null)} className="fx-tap w-full rounded-xl py-3" style={{ background: "transparent", border: `1px solid ${T.line}`, fontFamily: bodyFont, fontSize: 13, color: T.muted }}>
              {t("doneClose")}
            </button>
          </div>
        </div>
        ...
        <div className="fx-modal-card flex flex-col items-center text-center gap-5" style={{ width: "100%", maxWidth: 340 }}>
          <div style={{ width: 64, height: 64, clipPath: FACET, background: T.surfaceHi, border: `1px solid ${T.lineHi}`, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: `0 0 26px ${glow(0.3)}` }}>
          ...
          <div className="flex flex-col gap-2.5 w-full mt-1">
            {LAUNCH_STEPS.map((step, i) => {
              ...
              return (
                <div key={step.key} className="flex items-center gap-2.5 rounded-xl px-3 py-2.5" style={{
        ...
        <div className="fx-modal-card fx-view flex flex-col items-center text-center gap-4" style={{ width: "100%", maxWidth: 360, background: T.surface, border: `1px solid ${T.lineHi}`, borderRadius: 24, padding: 24 }}>
          <div style={{ width: 68, height: 68, borderRadius: "50%", overflow: "hidden", background: result.logoUrl ? `center/cover no-repeat url(${result.logoUrl})` : T.surfaceHi, border: `1px solid ${T.lineHi}`, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: `0 0 26px ${glow(0.32)}` }}>
          ...
          <div className="w-full flex flex-col gap-2 mt-1">
            <div className="flex items-center justify-between rounded-xl px-3 py-2.5" style={{ background: T.surfaceHi, border: `1px solid ${T.line}` }}>
            ...
            <div className="flex items-center justify-between rounded-xl px-3 py-2.5 gap-2" style={{ background: T.surfaceHi, border: `1px solid ${T.line}` }}>
            ...
            {result.explorerUrl && (
              <a href={result.explorerUrl} target="_blank" rel="noreferrer" className="fx-tap flex items-center justify-center gap-1.5 rounded-xl px-3 py-2.5" style={{ background: T.surfaceHi, border: `1px solid ${T.line}`, textDecoration: "none" }}>
          ...
          <div className="flex flex-col gap-2 w-full mt-2">
            <button
              ...
              className="fx-tap w-full rounded-xl py-3"
              style={{ background: PRISM, color: PRISM_TEXT, fontFamily: displayFont, fontWeight: 700, fontSize: 14, boxShadow: `0 0 22px ${glow(0.28)}` }}
            >
              {t("viewToken")}
            </button>
            <button onClick={() => onClose && onClose(result)} className="fx-tap w-full rounded-xl py-3" style={{ background: "transparent", border: `1px solid ${T.line}`, fontFamily: bodyFont, fontSize: 13, color: T.muted }}>
              {t("doneClose")}
            </button>
          </div>
        </div>
  ...
  return (
    <div className="fx-view flex flex-col gap-7 pb-36" style={{ position: "relative" }}>
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <div className="flex items-center gap-2 rounded-xl px-3.5 py-3" style={{ background: T.surface, border: `1px solid ${touched && !(parseFloat(form.buyAmount.replace(",", ".")) > 0) ? T.down : T.line}` }}>
          <input
        ...
        {(() => {
          ...
          return (
            <div className="flex items-center justify-between rounded-xl px-3.5 py-2.5" style={{ background: ink(0.06), border: `1px solid ${ink(0.2)}` }}>
  ...
  return (
    <div className="fx-view" style={{ position: "absolute", inset: 0, zIndex: 200, background: "#000000", display: "flex", flexDirection: "column", alignItems: "center", overflow: "hidden" }}>
      ...
      <div style={{ position: "relative", flex: 1, width: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24 }}>
        ...
        <div style={{ fontFamily: bodyFont, color: "#85858D", fontSize: 12.5, marginTop: 14 }}>{t("pinContinueNote")}</div>
      ...
      <div className="flex flex-col gap-0 pb-4">
        ...
        <div className="mt-5">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <StatBlock label="Created Tokens" value={myTokens.length} />
...
const FEE_PERCENT = 0.01; // 1% комиссии
  ...
  function readSavedTab() {
    try {
      if (typeof window !== "undefined") {
        const saved = window.localStorage.getItem("mintly_last_tab");
        if (saved && VALID_TABS.includes(saved)) return saved;
      }
    } catch (e) { /* localStorage unavailable */ }
    return "home";
  }
  const [view, setView] = useState(readSavedTab);
  const [tab, setTab] = useState(readSavedTab);
  const [token, setToken] = useState(null);
  const [connectModalOpen, setConnectModalOpen] = useState(false);
  const { height, insetBottom } = useTelegramViewport();

  // Remember which tab the person was on so a page refresh (or reopening
  // the mini app) drops them back where they left off instead of always
  // resetting to Home. Only the bottom-nav tab is persisted — not the
  // transient "token" detail view, since restoring the exact open token
  // would need re-fetching it by id after the live feed reloads.
  useEffect(() => {
    try { if (typeof window !== "undefined") window.localStorage.setItem("mintly_last_tab", tab); } catch (e) { /* localStorage unavailable */ }
  }, [tab]);
  ...
  async function runRealLaunch(req) {
    ...
    try {
      ...
      setLaunchProgress({
        ...
        error: null,
        result: {
          name: req.form.name.trim() || "Unnamed Token",
    ...
    <div style={{ background: T.bg, height, minHeight: height, width: "100%", maxWidth: 480, margin: "0 auto", fontFamily: bodyFont, position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ position: "relative", zIndex: 1, height: "100%", display: "flex", flexDirection: "column" }}>
        <div className="flex items-center justify-between px-4 pt-4 pb-2" style={{ flexShrink: 0 }}>
          ...
          <span style={{ fontFamily: displayFont, color: T.ice, fontSize: 22, fontWeight: 700, letterSpacing: "-0.01em" }}>Mintly</span>
          </button>
        ...
        <div className="no-scrollbar px-4" style={{ flex: 1, overflowY: "auto", minHeight: 0 }} key={view}>
          {view === "home" && <HomeView onGoTab={goTab} />}
          {view === "mempad" && <MempadView tokens={tokens} loading={tokensLoading} onOpen={openToken} onLaunch={() => goTab("create")} />}
        ...
        <div className="flex items-center justify-around px-2 py-4" style={{ borderTop: "1px solid rgba(255,255,255,0.06)", background: "rgba(8,8,10,0.86)", backdropFilter: "blur(24px) saturate(160%)", WebkitBackdropFilter: "blur(24px) saturate(160%)", flexShrink: 0, paddingBottom: insetBottom + 16 }}>
          {[
