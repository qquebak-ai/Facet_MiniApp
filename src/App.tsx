...
const FEE_PERCENT = 0.01; // 1% комиссии
  ...
  const TONAPI_KEY = "";
  // Показывает диагностическую плашку прямо в интерфейсе (chain кошелька,
  // какой tonapi-хост опрашивался, что он ответил). Пригождается внутри
  // Telegram, где консоль браузера недоступна. Выключите (false), когда
  // разберётесь с балансом — плашка не для продакшена.
  ...
  // В Telegram-вебвью нет доступа к консоли разработчика без remote
  // debugging, поэтому дублируем диагностику сюда — она рисуется прямо
  // на экране (см. DebugBadge ниже), пока не выключим DEBUG_TON.
  ...
  return (
    <div style={{ background: T.bg, height, minHeight: height, width: "100%", maxWidth: 480, margin: "0 auto", fontFamily: bodyFont, position: "relative", overflow: "hidden" }}>
      ...
      {DEBUG_TON && connected && (
        <div style={{
          position: "absolute", top: 8, left: 8, right: 8, zIndex: 999,
          background: "rgba(0,0,0,0.85)", border: "1px solid #444",
          borderRadius: 8, padding: "6px 9px", fontFamily: monoFont,
          fontSize: 9.5, lineHeight: 1.5, color: "#8f8", whiteSpace: "pre-wrap", wordBreak: "break-all",
        }}>
          {`TON DEBUG · chain=${tonDebug?.chain ?? "?"} (-239=mainnet, -3=testnet) · host=${tonDebug?.host ?? TONAPI_HOST}\nbalance=${tonBalance} raw=${tonDebug?.balanceRaw ?? "-"} status=${tonDebug?.status ?? "-"}${tonDebug?.error ? `\nerror=${tonDebug.error}` : ""}`}
        </div>
      )}
