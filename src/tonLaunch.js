...
export async function launchRealToken({
  ...
  try {
    // REVISION 6 — createJetton doesn't exist on the installed SDK
    // version (confirmed live: "createJetton is not a function"), so
    // back to deployJetton, which does exist and does get called (the
    // earlier trail showed it failing mid-flight with "Exceeded number
    // of retries", not "not a function" — proof the method itself is
    // real). The actual bug: deployJetton's internal "wait until
    // active" poll runs against the RAW `client`, which earlier
    // diagnosis already showed can serve a stuck/stale block height.
    // instrumentApi's tonapi.io-routed isContractDeployed override was
    // built for exactly this, but a previous revision built `sdk` from
    // the raw client instead and never actually wired it in — so the
    // fix was written but never applied. Applying it now for real
    // (`sdk` built from `sdkApi` below).
    ...
    fail(
      "Не удалось задеплоить jetton-контракт после нескольких попыток (внутренний поллинг deployJetton завис на нестабильном RPC-узле)",
      e
    );
  }
