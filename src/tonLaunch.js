...
export async function launchRealToken({
  ...
  // ---- premint the initial supply to the admin, also just queued —
  ...
  try {
    ...
    await minter.sendMint(batchSender, Address.parse(walletAddress), totalSupply * 10n ** decimals);
    log(`premint message queued: ${(totalSupply * 10n ** decimals).toString()} base units to ${walletAddress}`);
  } catch (e) {
  ...
  try {
    ...
    await batchSender.send({ to, value, body });
    log(`liquidity message queued: to ${to.toString()}, value ${value.toString()}`);
  ...
  try {
    ...
    log(`requesting a single wallet approval for ${messages.length} combined message(s) (deploy${batch.length > 1 ? " + premint" : ""}${batch.length > 2 ? " + liquidity pool" : ""})`);
    await tonConnectUI.sendTransaction({
      validUntil: Math.floor(Date.now() / 1000) + 300,
      network: network === "testnet" ? "-3" : "-239",
      messages,
  ...
  // ---- Stage 3: wait for pool to go active (skipped if the pool message
  // wasn't built earlier — see the try/catch above the combined send). ----
  onStage?.(3);
  if (poolAddress) {
    try {
