...
export async function launchRealToken({
  ...
  // testnet) instead of pinning to one hardcoded host.
  let endpoint;
  try {
    endpoint = await getHttpV4Endpoint({ network });
    console.log("[launch-debug] TON Access endpoint:", endpoint, "network:", network);
  } catch (e) {
    console.error("[launch-debug] getHttpV4Endpoint failed:", e);
    throw new Error(
      `Не удалось получить RPC-эндпоинт TON (${network}) через TON Access: ${e?.message || e}`
    );
  }
