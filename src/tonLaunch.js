/* ---------------------------------------------------------
   ...
   DEPLOY RETRY — why deployJetton is now wrapped in its own retry loop:
   Even with a confirmed-reachable RPC endpoint (the diagnostic probes all
   passed), deployJetton itself failed with a bare "Exceeded number of
   retries" — the SDK's own internal wait-for-the-minter-to-go-active
   polling gave up. That kind of failure is frequently transient (a slow
   block, one lagging node) rather than a real dead end, so deployJetton
   is now retried a few times with a short backoff before surfacing an
   error, instead of giving up on the very first attempt.
...
/* Retries an arbitrary async step a few times with a short, increasing
   delay before giving up. deployJetton's own internal "Exceeded number
   of retries" is frequently a transient hiccup (one slow/lagging node,
   a seqno that moved between calls) rather than a permanent failure —
   simply trying the whole call again a couple of times resolves it far
   more often than the SDK's own single attempt does. */
...
export async function launchRealToken({
  ...
  // shows exactly which underlying RPC call deployJetton was retrying —
  // the raw client is kept as `client` for our own direct calls (the
  // getAccountLite probe below, the STON.fi router later), only the copy
  // handed to AssetsSDK is wrapped.
  ...
  try {
    ...
    //
    // Wrapped in withRetries: even with a confirmed-reachable, correctly
    // configured client, deployJetton's own internal wait-for-active
    // polling has been observed to exhaust with a bare "Exceeded number
    // of retries" on a single attempt — most often a transient node lag
    // rather than a real failure, so the whole call gets a few extra
    // tries with backoff before this bubbles up to the user.
