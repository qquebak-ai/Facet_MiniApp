  ...
  return {
    ...
    async send(args) {
      ...
      try {
        ...
        warn(`sender.send: wallet did not approve / send failed: ${e?.message || e}`);
        throw e;
      }
    },
  };
...
/* Wraps a TonClient4 instance so every RPC method call it makes is logged
   ...
   stay correct. */
function instrumentApi(client, log, warn) {
  const ASYNC_METHODS = [
    ...
    get(target, prop, receiver) {
      ...
      return async function (...args) {
        ...
        try {
          ...
          warn(`client.${String(prop)}() failed: ${e?.message || e}`);
          throw e;
        }
      };
    },
...
async function withRetries(fn, { attempts = 3, delayMs = 2500, label = "", log = console.log, warn = console.warn } = {}) {
  ...
  for (let i = 1; i <= attempts; i++) {
    try {
      ...
      lastErr = e;
      warn(`${label || "step"} attempt ${i}/${attempts} failed: ${e?.message || e}`);
...
export async function launchRealToken({
  ...
  // instead of pinning to one hardcoded host — but its own node-manager
  // endpoint (ton.access.orbs.network/mngr/nodes) has been observed to be
  // unreachable from this environment (a SyntaxError parsing its response,
  // meaning the call never got valid JSON back at all — not a timeout, an
  // actually-broken response). Attempting it is now purely best-effort: on
  // ANY failure we go straight to the hardcoded fallback list and verify
  // it with our own raw probe (bypassing the library's own retry logic)
  // before trusting it — see pickWorkingEndpoint above.
  let taEndpoint = null;
  try {
    taEndpoint = await getHttpV4Endpoint({ network });
    log(`TON Access suggested endpoint: ${taEndpoint} (network: ${network})`);
  } catch (e) {
    warn(`getHttpV4Endpoint failed (ton-access node-manager unreachable), relying on fallback list: ${e?.message || e}`);
  }
  ...
  // AssetsSDK.createApi() was found to depend on the exact same broken
  // ton-access node-manager call as getHttpV4Endpoint above (identical
  // SyntaxError) — every attempt against it here is a guaranteed-to-fail
  // network round trip that only delays the launch before falling back
  // anyway. Going straight to the manually-built, already-probed
  // TonClient4 (`client`) skips that dead call entirely; deployJetton
  // works against a plain TonClient4 same as any other AssetsSDK call.
  //
  ...
  // is wrapped.
  const sdkApi = instrumentApi(client, log, warn);
  try {
    sdk = AssetsSDK.create({ api: sdkApi, sender, storage: null });
    log("AssetsSDK initialized (using the manually-built TonClient4 directly)");
  } catch (e) {
  ...
  onStage?.(1);
  // IMPORTANT — why this no longer calls sdk.deployJetton() directly:
  // deployJetton() is a combo call: it (a) sends the deploy message via
  // `sender`, THEN (b) internally polls the raw `client` it was built
  // with to confirm the contract went active, and throws its own
  // "Exceeded number of retries" if that internal poll gives up — a
  // SEPARATE poll from our own waitForActive below, using the raw
  // TonClient4 (the same fallback host Revision 3 already proved can
  // serve a stuck/stale block height). The trail confirmed this:
  // getAccountLite succeeded, then all 3 retries failed identically
  // with that exact message and NO "wallet approval" log line in
  // between — the failure was in the SDK's own post-send wait, not in
  // reaching the network or the wallet.
  //
  // Fix: open the jetton minter WITHOUT deploying (its address is
  // deterministic from code+data, known before any transaction), send
  // the deploy ourselves through the same sender used everywhere else,
  // and confirm it ONLY through our own waitForActive (Stage 2 below),
  // which already goes through tonapi.io instead of the flaky raw
  // client. The SDK's own internal wait is never invoked.
  let jettonMasterAddress;
  let jettonHandle;
  try {
    jettonHandle = sdk.openJetton({ uri: metadataUrl });
    jettonMasterAddress = jettonHandle.address;
    log(`computed jetton master address (pre-deploy): ${jettonMasterAddress.toString()}`);
  } catch (e) {
    fail(
      "Не удалось вычислить адрес jetton-контракта (проверьте версию @ton-community/assets-sdk — метод openJetton мог измениться)",
      e
    );
  }
  ...
  try {
    await withRetries(
      () => jettonHandle.sendDeploy(sender, {
        adminAddress: Address.parse(walletAddress),
        premintAmount: totalSupply * 10n ** decimals,
      }),
      { attempts: 3, delayMs: 3000, label: "deployJetton (send only, no internal wait)", log, warn }
    );
    log(`deploy message sent for: ${jettonMasterAddress.toString()}`);
  } catch (e) {
    fail(
      "Не удалось отправить транзакцию деплоя jetton-контракта (проверьте версию @ton-community/assets-sdk — метод sendDeploy мог измениться, либо кошелёк отклонил подтверждение)",
      e
    );
  }
