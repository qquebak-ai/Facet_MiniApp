/* ---------------------------------------------------------
   ...
   every request just times out — which is exactly what surfaced earlier
   as "Exceeded number of retries". @orbs-network/ton-access
   ...
   pinning you to one. If launches still fail after this change, the
   error will be a real, different message — not this generic retry
   exhaustion — which will tell us exactly where to look next.
--------------------------------------------------------- */
...
export async function launchRealToken({
  ...
  // testnet) instead of pinning to one hardcoded host — see the file
  // header note above for why this replaced the hardcoded tonhubapi.com
  // URLs.
  let endpoint;
  try {
    ...
    throw new Error(
      `Could not reach a TON ${network} RPC node via TON Access: ${e?.message || e}`
    );
  }
  ...
  // (source: ston-fi/dex-core and ston-fi/bug-bounty repos on GitHub —
