...
export async function launchRealToken({
  ...
  try {
    ...
    await batchSender.send({ to: Address.parse(treasuryAddress), value: toNano(mainTon.toFixed(9)) });
    await batchSender.send({ to: Address.parse(feeAddress), value: toNano(feeTon.toFixed(9)) });
