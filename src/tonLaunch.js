...
--------------------------------------------------------- */
function tonConnectSender(tonConnectUI, walletAddress, network = "mainnet", log = console.log, warn = console.warn) {
  return {
    address: Address.parse(walletAddress),
    async send(args) {
      // ton-core's internal() helper (used by JettonMinter.sendDeploy)
      // sets bounce:false for deploy messages (stateInit present, account
      // doesn't exist yet) and bounce:true otherwise. We were ignoring
      // args.bounce entirely and always encoding the address as
      // bounceable — a mismatch that's a plausible reason Tonkeeper's
      // local emulator couldn't simulate the transaction ("we don't know
      // what will happen next"). Respecting the real flag now.
      ...
      log(`sender.send: requesting wallet approval — to ${message.address} (bounceable=${bounceable}), amount ${message.amount}${message.stateInit ? ", with stateInit" : ""}`);
      try {
...
export async function launchRealToken({
  ...
  try {
    ...
    // sender and our own already-proven waitForActive. Any real error
    // here throws with its own message, uncaught by that retry().
    ...
    log("jetton minter deploy message sent");
  } catch (e) {
    fail(
      "Не удалось задеплоить jetton-контракт (низкоуровневый deploy через JettonMinter)",
      e
    );
  }
  ...
  // ---- Stage 2b: mint the initial supply to the admin (deploy alone
  // doesn't mint anything — this is a separate on-chain message) ----
  try {
    ...
    await minter.sendMint(sender, Address.parse(walletAddress), totalSupply * 10n ** decimals);
    log(`premint sent: ${(totalSupply * 10n ** decimals).toString()} base units to ${walletAddress}`);
  } catch (e) {
    fail("Токен задеплоен, но не удалось начать выпуск (premint)", e);
  }
  ...
  try {
    ...
    // address string this function returns.
    if (network === "testnet") {
    ...
    // Non-fatal: the jetton itself is already live on-chain at this point
    // (deploy + premint both succeeded above), so a broken STON.fi call —
    // wrong SDK/contract version, cell underflow, etc. — shouldn't throw
    // the whole launch away and force the user to redeploy a brand new
    // token from scratch just to retry the pool step. Log it, mark the
    // pool as not-yet-created (App.tsx already renders that state — see
    // `poolAddress: null` in its NEW_TOKEN_TEMPLATE), and let the launch
    // finish successfully. The pool can be created manually or retried
    // separately later without touching the jetton that already exists.
    const detail = describeError(e);
    console.error("[launch-debug]", "liquidity pool creation failed (non-fatal)", e);
    trail.push(
      `⚠ Токен создан (${jettonMasterAddress.toString()}), но пул ликвидности на STON.fi создать не удалось: ${detail}. Продолжаю без пула — его можно будет создать позже.`
    );
    warn(`pool creation failed, continuing without it: ${detail}`);
    poolAddress = null;
  }

  // ---- Stage 4: wait for pool to go active (skipped if pool wasn't created) ----
  if (poolAddress) {
    try {
      await waitForActive(poolAddress, network, { label: "STON.fi pool", log, warn });
    } catch (e) {
      // Also non-fatal for the same reason: the token exists either way.
      const detail = describeError(e);
      trail.push(
        `⚠ Пул отправлен в сеть, но не подтвердился вовремя: ${detail}. Токен уже задеплоен по адресу ${jettonMasterAddress.toString()} — проверьте пул вручную в STON.fi позже.`
      );
      warn(`pool did not confirm in time, continuing anyway: ${detail}`);
    }
  }
