...
function tonConnectSender(tonConnectUI, walletAddress, log = console.log, warn = console.warn) {
  return {
    ...
    async send(args) {
      const message = {
        address: args.to.toString({ urlSafe: true, bounceable: true }),
...
export async function launchRealToken({
  ...
  try {
    ...
    log("AssetsSDK initialized (unwrapped client, matching the official example exactly)");
  } catch (e) {
  ...
  try {
    // REVISION 5 — root cause: `deployJetton` is a legacy method name.
    // The currently published @ton-community/assets-sdk exposes
    // `createJetton` instead (its README documents createJetton(content,
    // options) with options.premint = { to, amount } — there's no
    // premintAmount field anymore). deployJetton's internal "wait until
    // active" polling is what produced the identical, zero-variation
    // "Exceeded number of retries" on all 3 attempts every time, with no
    // "wallet approval" log ever appearing — i.e. it never even reached
    // sending. createJetton is the maintained path, so this avoids that
    // broken internal poll entirely instead of patching around it.
    const jetton = await withRetries(
      () => sdk.createJetton(
        ...
        },
        {
          ...
          premint: { to: Address.parse(walletAddress), amount: totalSupply * 10n ** decimals },
        }
      ),
      { attempts: 3, delayMs: 3000, label: "createJetton", log, warn }
    );
    jettonMasterAddress = jetton.address;
    log(`jetton deployed at: ${jettonMasterAddress.toString()}`);
  } catch (e) {
    fail(
      "Не удалось задеплоить jetton-контракт после нескольких попыток (метод createJetton не сработал — проверьте версию @ton-community/assets-sdk, либо RPC-узел временно перегружен)",
      e
    );
  }
