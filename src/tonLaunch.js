...
export async function launchRealToken({
  ...
  // AssetsSDK gets its OWN api client, built through the SDK's own
  // createApi() factory — this is what every official ton-community/
  // assets-sdk example (deploy, mint, burn) actually uses, not a
  // manually-constructed TonClient4. deployJetton's internal polling/
  // retry logic is written against whatever createApi() returns; feeding
  // it a hand-built TonClient4 instead (even pointed at the same working
  // endpoint) is the most likely reason deployJetton's own retry budget
  // was exhausting where every other call against `client` succeeded.
  let sdkApi = client;
  try {
    sdkApi = await createApi(network);
    log("AssetsSDK createApi() succeeded");
  } catch (e) {
    warn(`createApi() failed, falling back to the manually-built TonClient4: ${e?.message || e}`);
  }
  try {
