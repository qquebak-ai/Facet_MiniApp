...
if (tg) {
  ...
  const наСтраницу = () => {
    ...
    слепок.просили = true;
    try { м.call(tg); } catch (e: any) { слепок.ошибка = String((e && e.message) || e).slice(0, 60); }
  };
  tg.onEvent?.("fullscreenChanged", () => { слепок.вышло = !!tg.isFullscreen; });
  tg.onEvent?.("fullscreenFailed", (со: any) => { слепок.ошибка = (со && со.error) || "отказ"; });
  наСтраницу();
  ...
  setTimeout(() => { if (!tg.isFullscreen && !слепок.ошибка) наСтраницу(); }, 700);
