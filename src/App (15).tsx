...
/* CyberGrid — full-app background layer: a quiet field of small 8-bit-style
   pixel stars that drift horizontally and exit off the edge of the screen
   (never falling — this deliberately avoids the "falling" motif used
   elsewhere in the app). Positions/speeds are seeded so the field doesn't
...
function CyberGrid({ forceDark }) {
  const stars = useMemo(() => {
    ...
    return Array.from({ length: 32 }).map((_, i) => {
      ...
      return {
        top: rand() * 100,
        size: 2 + Math.round(rand() * 2), // 2–4px chunky pixel squares
        dur: 20 + rand() * 26,
        delay: -rand() * 40,
        dir,
        color: rand() > 0.8 ? T.turquoise : T.ice,
        opacity: 0.3 + rand() * 0.45,
        twinkleDur: 1.8 + rand() * 2.4,
      };
    });
  }, []);
      ...
      {stars.map((s, i) => (
        <span
          ...
          style={{
            ...
            opacity: s.opacity, imageRendering: "pixelated",
            animation: `${s.dir > 0 ? "starDriftRight" : "starDriftLeft"} ${s.dur}s linear ${s.delay}s infinite, starTwinkle ${s.twinkleDur}s ease-in-out ${s.delay}s infinite`,
          }}
        />
      ))}
    </div>
    ...
    <div className="fx-view flex flex-col gap-4 pb-4" style={{ position: "relative" }}>
      ...
      {tab === "chart" && (
        <>
          ...
          <div className="rounded-[22px] overflow-hidden" style={{ background: "#000000", border: `1px solid ${T.line}`, padding: 2 }}>
  ...
  return (
    <div className="fx-modal-back" style={{ position: "absolute", inset: 0, zIndex: 60, background: "rgba(0,0,0,0.75)", backdropFilter: "blur(4px)", display: "flex", alignItems: "flex-end" }} onClick={onClose}>
      <div
        ...
        style={{
          width: "100%", background: T.surface, border: `1px solid ${T.lineHi}`, borderRadius: "24px 24px 0 0",
          maxHeight: `calc(84% - ${insetBottom}px)`, display: "flex", flexDirection: "column", overflow: "hidden",
      ...
      >
        ...
        <div className="no-scrollbar" style={{ padding: "0 22px", paddingBottom: 22 + insetBottom, overflowY: "auto", flex: 1, minHeight: 0 }}>
          {body}
        </div>
      </div>
    </div>
